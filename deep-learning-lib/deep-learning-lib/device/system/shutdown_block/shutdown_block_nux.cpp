...
// File header:
#include "deep-learning-lib/device/system/shutdown_block/shutdown_block_nux.hpp"
...
bool ShutdownBlock::check_systemd_version(void) const {
  ...
  if (output.empty()) {
    ERR(L"An error has been triggered from the `check_output(busctl "
        L"get-property org.freedesktop.login1 /org/freedesktop/login1 "
        L"org.freedesktop.login1.Manager PreparingForSleep)` function.");
    return false;
  }
  ...
  if (char_pos == std::wstring::npos) {
    ERR(L"An error has been triggered from the `find_first_of( )` function.");
    return false;
  }
  ...
  try {
    ...
    ERR(L"An error has been triggered from the `std::stoi(%ls) -> %ls` "
        L"function.",
        output.c_str(), e.what());
    return false;
  }
  ...
  if (systemd_ver < 220) {
    ERR(L"Systemd current version %d need to be update to the version 220 or "
        L"greater.",
        systemd_ver);
    return false;
  }
...
bool ShutdownBlock::block(bool const use_ctrl_handler) {
  if (this->check_systemd_version() == false) {
    ERR(L"An error has been triggered from the `check_systemd_version()` "
        L"function.");
    return false;
  }
  ...
  if (this->_sd_bus == NULL &&
    ...
    ERR(L"An error has been triggered from the `sd_bus_open_system() -> %d` "
        L"function.",
        ret_code);
  ...
  {
    ERR(L"An error has been triggered from the `sd_bus_call_method() -> (%d, "
        L"%ls)` function.",
...
bool ShutdownBlock::peek_message(void) {
  ...
  {
    ERR(L"An error has been triggered from the `sd_bus_get_property() -> (%d, "
        L"%ls)` function.",
        ret_code, this->_sd_bus_error.message);
  ...
  if ((ret_code =
    ...
    ERR(L"An error has been triggered from the `sd_bus_message_read() -> %d` "
        L"function.",
        ret_code);
    return false;
  }
