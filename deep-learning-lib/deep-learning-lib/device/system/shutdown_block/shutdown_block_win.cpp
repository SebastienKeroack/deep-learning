...
// File header:
#include "deep-learning-lib/device/system/shutdown_block/shutdown_block_win.hpp"
...
LRESULT CALLBACK CallBack_WndProc(HWND hwnd, UINT msg, WPARAM wparam,
  ...
  switch (msg) {
    ...
    case WM_CREATE:  // 1
      ShutdownBlockReasonCreate(hwnd, L"Closing in progress...");
      break;
    case WM_DESTROY:  // 2
      PostQuitMessage(0);
      break;
...
BOOL ShutdownBlock::initiate_instance(HINSTANCE handle, int nCmdShow) {
  ...
  if ((ret_code = UpdateWindow(this->_hwnd)) == FALSE) {
    ERR(L"An error has been triggered from the `UpdateWindow() -> %d` "
        L"function.",
        ret_code);
    return FALSE;
  }
...
bool ShutdownBlock::block(bool const use_ctrl_handler) {
  ...
  if (use_ctrl_handler && (ret_code = SetConsoleCtrlHandler(
                               ...
                               TRUE)) == FALSE) {
    ERR(L"An error has been triggered from the `SetConsoleCtrlHandler() -> "
        "%d` function.",
        ret_code);
    return false;
  }
  ...
  if ((ret_code = this->initiate_instance(hinstance, SW_HIDE)) == FALSE) {
    ERR(L"An error has been triggered from the `initiate_instance() -> %d` "
        L"function.",
        ret_code);
    return false;
  }
...
bool ShutdownBlock::unblock(void) {
  ...
  if ((ret_code = ShutdownBlockReasonDestroy(this->_hwnd)) == FALSE) {
    ERR(L"An error has been triggered from the `ShutdownBlockReasonDestroy() "
        L"-> %d` function.",
        ret_code);
    return false;
  }
...
bool ShutdownBlock::peek_message(void) {
  ...
  while ((ret_code = PeekMessageW(&msg, this->_hwnd, 0, 0, PM_REMOVE)) !=
         FALSE) {
    if (ret_code == FAIL) {
      ERR(L"An error has been triggered from the `PeekMessage() -> %d` "
          L"function.",
          ret_code);
      return false;
    }
