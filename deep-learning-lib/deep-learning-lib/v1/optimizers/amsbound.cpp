...
namespace DL::v1 {
void Model::Update_Parameters__AMSBound(size_t const batch_size,
                                        size_t const training_size,
                                        size_t const str, size_t const end) {
  if (this->use_mp && this->is_mp_initialized) {
    this->Update_Parameters__AMSBound__OpenMP(batch_size, training_size, str,
                                              end);
  } else {
    this->Update_Parameters__AMSBound__Loop(batch_size, training_size, str,
                                            end);
  }
}

void Model::Update_Parameters__AMSBound__Loop(size_t const batch_size,
                                              size_t const training_size,
                                              size_t const str,
                                              size_t const end) {
  size_t i;
  ...
  real const *const mask(this->ptr_array_mask_regularized_parameters),
      ...
      lr_lowerb_t(
          ...
          (1_r -
           1_r / (this->learning_gamma * this->optimizer_time_step + 1_r))),
      lr_upperb_t(
          lr_scale * this->learning_rate_final *
  ...
  if (wd != 0_r) {
    for (i = str; i != end; ++i) {
      ...
      biased_secod_moment_hat_tm1 = biased_secod_moments_hat_tm1[i];
      biased_secod_moments_hat_tm1[i] = biased_secod_moment_hat_tm1 =
          std::max(biased_secod_moment_hat_tm1, biased_secod_moment);

      lr_t_clipped = clip(lr_t / (sqrt(biased_secod_moment_hat_tm1) + epsilon),
                          lr_lowerb_t, lr_upperb_t);
...
void Model::Update_Parameters__AMSBound__OpenMP(size_t const batch_size,
                                                size_t const training_size,
                                                size_t const str,
                                                size_t const end) {
  ...
  real *const derivatives(this->ptr_array_derivatives_parameters),
      *const biased_first_moments_tm1(
      ...
      biased_secod_moment_hat_tm1(0_r), lr_t_clipped(0_r);
