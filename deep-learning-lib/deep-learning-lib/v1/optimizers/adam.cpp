...
namespace DL::v1 {
void Model::Update_Parameters__Adam(size_t const batch_size,
                                    size_t const training_size,
                                    size_t const str, size_t const end) {
  if (this->use_mp && this->is_mp_initialized) {
    this->Update_Parameters__Adam__OpenMP(batch_size, training_size, str, end);
  } else {
    this->Update_Parameters__Adam__Loop(batch_size, training_size, str, end);
  }
}

void Model::Update_Parameters__Adam__Loop(size_t const batch_size,
                                          size_t const training_size,
                                          size_t const str, size_t const end) {
  size_t i;
  ...
  if (wd != 0_r) {
    for (i = str; i != end; ++i) {
      ...
      parameters[i] -=
          lr_t * biased_first_moment / (sqrt(biased_secod_moment) + epsilon) +
...
void Model::Update_Parameters__Adam__OpenMP(size_t const batch_size,
                                            size_t const training_size,
                                            size_t const str,
                                            size_t const end) {
