...
bool CrossVal::Initialize__Fold(bool const use_shuffle_received,
  ...
  if (this->Set__Desired_K_Fold(number_k_fold_received,
    ...
    ERR(
        ...
        "\"Set__Desired_Data_Per_Batch(%zu, %zu)\" function.", number_k_fold_received,
        number_k_sub_fold_received);

    return false;
  }

  if ((this->ptr_array_stochastic_index =
           new size_t[this->p_n_data]) == nullptr) {
...
bool CrossVal::Set__Desired_K_Fold(size_t const number_k_fold_received,
  ...
  if (this->p_n_data == 0_UZ) {
    ...
    ERR(
        L"K-fold (%zu) must not be greater than total number of "
        "data (%zu).", number_k_fold_received,
        this->DatasetV1::get_n_data());
  ...
  if (this->ptr_array_inputs_array_k_fold == nullptr) {
    if ((this->ptr_array_inputs_array_k_fold =
             new real const *[(number_k_fold_received - 1_UZ) *
                           this->number_examples_per_fold]) == nullptr) {
      ERR(L"Can not allocate %zu bytes.",
  ...
  if (this->ptr_array_outputs_array_k_fold == nullptr) {
    if ((this->ptr_array_outputs_array_k_fold =
             new real const *[(number_k_fold_received - 1_UZ) *
                           this->number_examples_per_fold]) == nullptr) {
      ERR(L"Can not allocate %zu bytes.",
             (number_k_fold_received - 1_UZ) * this->number_examples_per_fold *
  ...
  if (this->ptr_array_inputs_array_validation == nullptr) {
    if ((this->ptr_array_inputs_array_validation =
             new real const *[this->DatasetV1::get_n_data() -
                           this->number_examples_training]) == nullptr) {
      ERR(L"Can not allocate %zu bytes.",
             this->DatasetV1::get_n_data() -
                 this->number_examples_training * sizeof(real const *));
  ...
  if (this->ptr_array_outputs_array_validation == nullptr) {
    if ((this->ptr_array_outputs_array_validation =
             new real const *[this->DatasetV1::get_n_data() -
                           this->number_examples_training]) == nullptr) {
      ERR(L"Can not allocate %zu bytes.",
             this->DatasetV1::get_n_data() -
  ...
  if (number_k_sub_fold_received > this->number_examples_training) {
    ERR(
        L"K-sub-fold (%zu) > (%zu) amount of data from the training "
        "set.", number_k_sub_fold_received,
        this->number_examples_training);
    ...
    if (this->Increment_Fold(tmp_fold_index)) {
      for (tmp_sub_fold_index = 0_UZ;
        ...
        if (this->Increment_Sub_Fold(tmp_sub_fold_index)) {
          ...
          ERR(
              L"An error has been triggered from the "
              "\"Increment_Sub_Fold(%zu)\" function.",
              tmp_sub_fold_index);
      ...
      ERR(
          L"An error has been triggered from the "
          "\"Increment_Fold(%zu)\" function.", tmp_fold_index);
