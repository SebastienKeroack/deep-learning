...
MatrixXT &diag_part(MatrixXT out) {
  ...
  size_t const smallest_length(out.rows() < out.cols() ? out.rows()
                                                       : out.cols());

  for (size_t i(0_UZ); i != smallest_length; ++i) out(i, i) = diag(i);
...
void Model::weights_initialize_orthogonal(size_t const rows, size_t const cols,
  ...
  size_t const minval(std::min(rows, cols)), maxval(std::max(rows, cols)),
      length(rows * cols);
