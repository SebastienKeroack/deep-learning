...
bool Model::Initialize__LSUV(size_t const max_n_trials,
  ...
  if (max_batch_size == 0_UZ) {
    ...
    ERR(L"Variance target (%f) can not be less than zero.", cast(var_target));
    return false;
  } else if (var_tolerance < 0_r) {
    ERR(L"Variance tolerance (%f) can not be less than zero.",
        cast(var_tolerance));
    return false;
  }
