...
namespace DL::v1 {
real Model::Get__Regularization__SRIP(void) const {
  return (this->regularization__srip);
}

bool Model::Set__Regularization__SRIP(
    real const regularization__srip_received) {
  if (regularization__srip_received < 0_r) {
    ERR(L"L2 regularization (%f) less than zero.",
        cast(regularization__srip_received));

    return false;
  } else if (regularization__srip_received > 1_r) {
  ...
  if (this->regularization__srip != regularization__srip_received) {
    bool const tmp_use_regularization(this->Use__Regularization_Parameter());

    this->regularization__srip = regularization__srip_received;

    if (tmp_use_regularization == false &&
        regularization__srip_received != 0_r) {
    ...
    if (this->is_cu_initialized) {
      this->cumodel->Set__Regularization__L2(regularization__srip_received);
    }
#endif
  }
...
void Model::Update_Derivative_Weight__Regularization__SRIP(
...
void Model::Update_Derivative_Weight__Regularization__SRIP__Loop(
    size_t const str, size_t const end, size_t const batch_size) {
  real *derivative_it(this->ptr_array_derivatives_parameters + str);
  real const *const last_derivative(derivative_it + end),
      *mask_it(this->ptr_array_mask_regularized_parameters + str);

  var const *parameter_it(this->ptr_array_parameters + str);

  for (; derivative_it != last_derivative;
       ++derivative_it, ++parameter_it, ++mask_it) {
    *derivative_it +=
        *mask_it * cast(*parameter_it) * this->regularization__srip;
  }
}

void Model::Update_Derivative_Weight__Regularization__SRIP__OpenMP(
    size_t const str, size_t const end, size_t const batch_size) {
  ...
  var const *parameters(this->ptr_array_parameters + str);

#pragma omp parallel for schedule(static)
  for (i = static_cast<int>(str); i < end_; ++i) {
    derivatives[i] +=
        mask[i] * cast(parameters[i]) * this->regularization__srip;
  }
}
}  // namespace DL
