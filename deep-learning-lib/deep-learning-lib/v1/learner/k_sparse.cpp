...
bool Model::Set__K_Sparsity(size_t const index_layer_received, size_t const k_sparsity_received)
{
...
bool Model::Set__K_Sparsity(Layer *const ptr_layer_received, size_t const k_sparsity_received)
{
    ...
    else if(static_cast<size_t>(static_cast<real>(k_sparsity_received) * ptr_layer_received->alpha_sparsity) > *ptr_layer_received->ptr_number_outputs)
    {
        ERR(L"k-sparse cannot be %zu because an overflow (%zu * %f = %zu) occur (limit=%zu).",
                                 k_sparsity_received,
                                 k_sparsity_received,
                                 ptr_layer_received->alpha_sparsity,
                                 static_cast<size_t>(static_cast<real>(k_sparsity_received) * ptr_layer_received->alpha_sparsity),
    ...
    if(ptr_layer_received->k_sparsity == 0_UZ && k_sparsity_received != 0_UZ)
    {
        ...
        if(++this->total_k_sparse_layers == 1_UZ && this->Allocate__Sparse_K_Filter() == false)
        {
            ERR(L"An error has been triggered from the \"Allocate__Sparse_K_Filter()\" function.",);
    ...
    else if(ptr_layer_received->k_sparsity != 0_UZ && k_sparsity_received == 0_UZ)
    {
...
bool Model::Set__Alpha_Sparsity(size_t const index_layer_received, real const alpha_sparsity_received)
{
    if(index_layer_received >= this->total_layers)
    {
        ERR(L"Layer received (%zu) overflow the number of layers (%zu) in the neural network.",
...
bool Model::Set__Alpha_Sparsity(Layer *const ptr_layer_received,real const alpha_sparsity_received)
{
    if(ptr_layer_received == nullptr)
    {
        ERR(L"\"ptr_layer_received\" is a nullptr.",);
    ...
    else if(alpha_sparsity_received < 0_r)
    {
        ERR(L"alpha k-sparse (%f) cannot be less than zero.",
                                 cast(alpha_sparsity_received));

        return false;
    }
    else if(static_cast<size_t>(static_cast<real>(ptr_layer_received->k_sparsity) * alpha_sparsity_received) > *ptr_layer_received->ptr_number_outputs)
    {
        ERR(L"alpha k-sparse cannot be %f because an overflow (%zu * %f = %zu) occur (limit=%zu).",
                                 cast(alpha_sparsity_received),
                                 ptr_layer_received->k_sparsity,
                                 cast(alpha_sparsity_received),
                                 static_cast<size_t>(static_cast<real>(ptr_layer_received->k_sparsity) * alpha_sparsity_received),
                                 *ptr_layer_received->ptr_number_outputs);
...
void Model::Assign__Sparsity_Activities(size_t const number_threads_received)
{
    ...
    std::pair<size_t, var> *tmp_ptr_array_k_sparses(this->ptr_array_k_sparse_activities);

    // Assign array position to each layers.
    for(; layer_it != last_layer; ++layer_it)
    {
        layer_it->ptr_array_k_sparse_activities = tmp_ptr_array_k_sparses;
        ...
        switch(layer_it->type_layer)
        {
            ...
            case LAYER::LSTM:
            case LAYER::RESIDUAL: tmp_ptr_array_k_sparses += number_threads_received * *layer_it->ptr_number_outputs; break;
            case LAYER::FULLY_CONNECTED:
            case LAYER::FULLY_CONNECTED_RECURRENT: tmp_ptr_array_k_sparses += number_threads_received * static_cast<size_t>(layer_it->ptr_last_AF_unit - layer_it->ptr_array_AF_units); break;
            case LAYER::FULLY_CONNECTED_INDEPENDENTLY_RECURRENT: tmp_ptr_array_k_sparses += number_threads_received * static_cast<size_t>(layer_it->ptr_last_AF_Ind_recurrent_unit - layer_it->ptr_array_AF_Ind_recurrent_units); break;
            default:
...
void Model::Sparse_K_Filter(size_t const time_step_index_received,
...
void Model::Sparse_K_Filter__Loop(size_t const time_step_index_received,
                                                                  size_t const batch_size,
                                                                  size_t const input_unit_size_received,
                                                                  size_t const k_sparsity_received,
                                                                  std::pair<size_t, var> *const ptr_array_k_sparses_received,
                                                                  var *const ptr_array_inputs_received)
{
    ...
    // Custom sorting.
    auto tmp_Sort_Pair([](std::pair<size_t, var> &a_received, std::pair<size_t, var> &b_received) -> bool { return a_received.second < b_received.second; });
    ...
    for(tmp_example_index = 0_UZ; tmp_example_index != batch_size; ++tmp_example_index)
    {
        ...
        for(tmp_unit_index = 0_UZ; tmp_unit_index != k_sparsity_received; ++tmp_unit_index)
        {
        ...
        {
            if(ptr_array_k_sparses_received[0].second < ptr_array_inputs_received[tmp_unit_data_timed_index + tmp_unit_index])
            {
                ptr_array_k_sparses_received[0].first = tmp_unit_index;

                ptr_array_k_sparses_received[0].second = ptr_array_inputs_received[tmp_unit_data_timed_index + tmp_unit_index];
...
void Model::Sparse_K_Filter__OpenMP(size_t const time_step_index_received,
                                                                        size_t const batch_size,
                                                                        size_t const input_unit_size_received,
                                                                        size_t const k_sparsity_received,
                                                                        std::pair<size_t, var> *const ptr_array_k_sparses_received,
                                                                        var *const ptr_array_inputs_received)
{
    ...
    size_t const tmp_unit_timed_index(input_unit_size_received * time_step_index_received);
    size_t tmp_unit_index(0_UZ),
              tmp_unit_data_timed_index(0_UZ);
    
    std::pair<size_t, var> *tmp_ptr_array_k_sparses(nullptr);
    ...
    for(tmp_example_index__int = 0; tmp_example_index__int < batch_size_received__int; ++tmp_example_index__int)
    {
        ...
        for(tmp_unit_index = k_sparsity_received; tmp_unit_index != input_unit_size_received; ++tmp_unit_index)
        {
            if(tmp_ptr_array_k_sparses[0].second < ptr_array_inputs_received[tmp_unit_data_timed_index + tmp_unit_index])
            {
                tmp_ptr_array_k_sparses[0].first = tmp_unit_index;

                tmp_ptr_array_k_sparses[0].second = ptr_array_inputs_received[tmp_unit_data_timed_index + tmp_unit_index];
