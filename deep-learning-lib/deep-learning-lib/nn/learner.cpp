...
// File header:
#include "deep-learning-lib/nn/learner.hpp"

// Deep learning lib:
#include "deep-learning-lib/data/enum/hierarchy.hpp"
...
bool Checkpoint::save(void) {
  ...
  if (wopen(file, this->path_name, std::wios::out | std::wios::binary) ==
      false) {
    ERR(L"An error has been triggered from the `File::wopen(%ls)` "
        L"function.",
        this->path_name.c_str());
    return false;
  }
  ...
  if (wclose(file, this->path_name) == false) {
    ERR(L"An error has been triggered from the `File::wclose(%ls)` "
        L"function.",
        this->path_name.c_str());
    return false;
  }
...
bool Checkpointer::load(void) {
  ...
  if (this->model->load(path_weights, path_configs, allowable_host_mem) ==
      false) {
    ERR(L"An error has been triggered from the `Model::load()` "
        L"function.");
    return false;
  }

  if (this->model->set_mp(this->model->use_mp) == false) {
    ERR(L"An error has been triggered from the `Model::set_mp()` "
        L"function.");
    return false;
  }

  if (this->model->set_cu(this->model->use_cu, allowable_devc_mem) == false) {
    ERR(L"An error has been triggered from the `Model::set_cu()` "
        L"function.");
    return false;
  }
...
bool Checkpointer::save(void) {
  ...
  if (create_directory(path_ckpt) == false) {
    ERR(L"An error has been triggered from the `create_directory(%ls)` "
        L"function.",
        path_ckpt.c_str());
    return false;
  }
  ...
  if (this->model->save_params(path_weights) == false) {
    ERR(L"An error has been triggered from the `Model::save_params(%ls)` "
        L"function.",
        path_weights.c_str());
    return false;
  }
  ...
  if (this->model->save_spec_params(path_configs) == false) {
    ERR(L"An error has been triggered from the `Model::save_spec_params(%ls)` "
        L"function.",
        path_configs.c_str());
    return false;
  }
...
void Checkpointer::rotate(void) {
  std::vector<
      std::tuple<std::filesystem::path, std::chrono::system_clock::duration>>
      ckpts;

  for (auto const &entry :
       std::filesystem::directory_iterator(this->ckpt_dir)) {
    if (entry.is_directory() == false) continue;
    if (entry.path().compare(L"ckpt-") == 0) continue;

    ckpts.push_back(std::make_tuple(
        entry.path(), entry.last_write_time().time_since_epoch()));
  }

  if (ckpts.size() <= this->max_to_keep) return;

  std::sort(ckpts.begin(), ckpts.end(), [](auto const &t1, auto const &t2) {
    return get<1>(t1) > get<1>(t2);
  });

  for (int i(this->max_to_keep); i != ckpts.size(); ++i)
    delete_directory(get<0>(ckpts[i]));
}
...
std::wstring Checkpointer::read_model_checkpoint_path(void) {
  ...
  if (parse_from_file(file, L"model_checkpoint_path:", ckpt_name) == false) {
    ERR(L"An error has been triggered from the `File::parse_from_file()` "
        L"function.",
        this->ckpt_file.c_str());
    return L"";
  }
...
Interval::Interval(float early_stop_pct, int n_iters, int evalt, int log_g,
                   int save) {
  this->early_stop =
      std::max<int>(1, static_cast<int>(n_iters * early_stop_pct));
  this->evalt = evalt;
  this->log_g = log_g;
...
void Learner::_initialize_dirs(void) {
  ...
  if (this->dirs->initialize() == false)
    throw std::runtime_error(
        "An error has been triggered from the `Directories::initialize()` "
        "function.");
}
