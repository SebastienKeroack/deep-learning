...
// Deep learning lib:
#include "deep-learning-lib/data/dataset/mnist.hpp"
...
class MNISTLearner : public Learner {
 protected:
  void _create_model(void) {
    ...
    for (size_t i(0_UZ); i != N_units.size(); ++i) {
      layer_params.type_layer = v1::LAYER::FULLY_CONNECTED;
      layer_params.unit_parameters[0] = N_units[i];
      layers_params.push_back(layer_params);
    }
  ...
  void _create_datasets(void) {
    for (ENV::TYPE const &env : {ENV::TRAIN, ENV::VALID}) {
 ...
 public:
  MNISTLearner(std::wstring const &name = L"mnist",
               int n_iters = flags->get(L"n_iters", 0.0))
