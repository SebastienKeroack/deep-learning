...
bool Experimental::setup_cudevice(void) {
  ...
  if (this->_datasets->Initialize__CUDA() == false) {
    ERR(L"Function \"Initialize__CUDA()\" return false.");
    return false;
  }
...
void Experimental::start(void) {
  for (unsigned int i = 0u; i != this->n_models; ++i) {
    if (this->run(i) == false) {
      ERR(L"Function \"run(%u)\" return false.", i);
      break;
    }
  }
}
...
bool Experimental::run(unsigned int model_id) {
  ...
  if (this->load_spec_params) {
    ...
    if (model->compile(layers_params.size(), this->_datasets->get_seq_w(),
      ...
      ERR(L"An error has been triggered from the \"compile()\" "
          "function.", );
    ...
    if (this->tied_params) {
      for (ep = 1_UZ; ep != layers_params.size() - 1_UZ; ++ep) {
        if (model->Set__Tied_Parameter(ep, true) == false) {
          ERR(L"An error has been triggered from the "
              "\"Set__Tied_Parameter(%zu, true)\" function.",
              ep);
    ...
    for (ep = 1_UZ; ep != layers_params.size() - 1_UZ; ++ep) {
      if (model->set_layer_activation_function(ep, this->type_af_hidden) ==
          false) {
        ERR(L"An error has been triggered from the "
            "\"set_layer_activation_function(%zu, %u)\" function.",
    ...
    if (model->Set__Regularization__L1(this->l1) == false) {
      ERR(L"An error has been triggered from the "
    ...
    if (model->Set__Regularization__L2(this->l2) == false) {
      ERR(L"An error has been triggered from the "
          "\"Set__Regularization__L1(%f)\" function.",
          this->l2);
    ...
    // TODO: Each layer having a max-norm constraints regularization
    // parameter.
    if (model->Set__Regularization__Weight_Decay(this->weight_decay) == false) {
      ERR(L"An error has been triggered from the "
    ...
    if (model_id == 0_UZ) {
      ...
      switch (this->type_weight_initializer) {
        ...
        case INITIALIZER::UNIFORM:
          ...
          break;
        default:
          ERR(L"Weights initializer (%u | %ls) is not managed in the switch.",
      ...
      if (this->print_params) {
        for (size_t w = 0_UZ; w != model->total_parameters; ++w)
          INFO(L"W[%zu]: %f", w, this->_initial_parameters[w]);
      }
    } else {
      ...
      switch (this->type_dropout) {
        ...
        default:
          ERR(L"Dropout type (%d | %ls) is not managed in the "
              "switch. Need to be the fully connected layer or one of its "
              "variant.",
              this->type_dropout,
              LAYER_DROPOUT_NAME[this->type_dropout].c_str());
          return false;
  ...
  if (model->set_cu(this->use_cuda, this->_mem_allocate) == false) {
    ERR(L"An error has been triggered from the \"set_cu(%ls, "
        "%zu)\" function.",
        this->use_cuda ? "true" : "false", this->_mem_allocate);
  ...
  if (this->use_cuda &&
    ...
    ERR(L"An error has been triggered from the "
        "\"Initialize__CUDA__Thread(ptr)\" function.", );
  ...
  if (this->use_adept) {
    INFO(L"");
    INFO(
        L"Adept: Evaluate %zu example(s) from the "
        "testing set.",
  ...
  if (this->use_infer_set) {
    INFO(L"");
    INFO(
        L"Evaluate %zu example(s) from the "
        "testing "
        "set.",
        this->_datasets->get_dataset(ENV::TESTG)->DatasetV1::get_n_data());
          ...
          if (this->infer_set_print_x) {
            if (this->input_image_size == 0_UZ) {
              ...
              for (tmp_input_index = 0_UZ; tmp_input_index != tmp_number_inputs;
                ...
                INFO(L"%.0f ",
                     round(cast(this->_datasets->get_inp(
                         tmp_example_index)[tmp_time_step * tmp_number_inputs +
                                            tmp_input_index])));
              }
            }
  ...
  INFO(L"");
  INFO(
      L"Optimize %zu example(s) from the training set for "
      "%zu epoch(s).",
  ...
  if (this->update_bn_final) {
    INFO(L"");
    INFO(
        L"Update batch normalization on %zu example(s) "
...
bool Experimental::check_attributes(void) {
  ...
  switch (this->type_fully_connected) {
    ...
    case LAYER::FULLY_CONNECTED_INDEPENDENTLY_RECURRENT:
      break;
    default:
      ERR(L"The `switch` require a dense layer or one of its variant. Not a "
          L"(%u "
          "| %ls).",
          this->type_fully_connected, LAYER_NAME[this->type_fully_connected]);
      return false;
  }
  ...
  switch (this->type_pooling_layer) {
    ...
    case LAYER::MAX_POOLING:
      break;
    default:
      ERR(L"The `switch` require a pooling layer. Not a (%u | %ls).",
          this->type_pooling_layer, LAYER_NAME[this->type_pooling_layer]);
      return false;
  }
...
[[deprecated("Untested.")]] void run_custom(void) {
  ...
  if (experimental.check_attributes() == false) {
    ERR(L"Function \"check_attributes()\" return false.");
    return;
  }
  ...
  if (experimental.setup_datasets() == false) {
    ERR(L"Function \"setup_datasets()\" return false.");
    return;
  }

#ifdef COMPILE_CUDA
  if (experimental.setup_cudevice() == false) {
    ERR(L"Function \"setup_cudevice()\" return false.");
    return;
  }
#endif  // COMPILE_CUDA
