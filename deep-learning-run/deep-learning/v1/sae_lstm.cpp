...
bool preprocess_sae(class Datasets *const datasets) {
  ...
  for (i = 0_UZ; i != 8_UZ; ++i) {
    min_inp =
        (std::min)(train_set->get_min(0_UZ, train_set->DatasetV1::get_n_data(),
                                      i, DATA::INPUT),
                   min_inp);

    max_inp =
        (std::max)(train_set->get_max(0_UZ, train_set->DatasetV1::get_n_data(),
                                      i, DATA::INPUT),
                   max_inp);
  }
...
bool sae_lstm(void) {
  ...
  if (f_models.initialize_dirs(f_model_name) == false) {
    ERR(L"An error has been triggered from the "
        "\"initialize_dirs(%ls, %ls)\" function.",
  ...
  if (f_models.initialize_datasets(&datasets_params) == false) {
    ERR(L"An error has been triggered from the "
        "\"initialize_datasets(ptr)\" function.");
    return false;
  }
  ...
  if (s_models.load_model(HIERARCHY::TRAINER, allowable_host_mem,
    ...
    ERR(L"An error has been triggered from the "
        "\"load_model(%ls, %zu, %zu, false)\" function.",
    ...
    ERR(L"An error has been triggered from the "
        "\"load_model(%ls, %zu, %zu, true)\" function.",
  ...
  if (s_model->Set__Input_Mode(true) == false) {
    ERR(L"An error has been triggered from the "
        "\"Set__Input_Mode(true)\" function.", );
    return false;
  } else if (s_model->Set__Output_Mode(false) == false) {
    ERR(L"An error has been triggered from the "
        "\"Set__Output_Mode(false)\" function.", );
    return false;
  }
  ...
  if (l_dsets->get_n_data() == 0_UZ) {
    ...
    ERR(L"The number of input(s) (%zu) differ from the number of "
        "output(s) from the autoencoder (%zu).",
  ...
  if (train) {
    ...
    s_train_time = parse_discrete<size_t>(1_UZ, L"Time in seconds: ");

    INFO(L"");
    INFO(L"LSTM optimization time.");
    INFO(L"default=120 (seconds).");
    l_train_time = parse_discrete<size_t>(1_UZ, L"Time in seconds: ");
  ...
  INFO(L"Range[1 , - ].");
  INFO(L"default=24.");
  acc_avg_size = parse_discrete<size_t>(0_UZ, L"Time average: ");
  ...
  for (i = 0_UZ; i != n_data; ++i) {
    ...
    // Compute LSTM loss.
    {
      ...
      *l_model->ptr_array_number_loss += i * f_dsets->get_n_out();
      ...
      // Print output(s)
      for (int k(0); k != f_dsets->get_n_out(); ++k) {
        INFO(L"Output[%d]: %f", k,
