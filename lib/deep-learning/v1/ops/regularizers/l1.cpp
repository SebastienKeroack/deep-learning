...
namespace DL::v1 {
real Model::get_l1(void) const { return this->regularization__l1; }

bool Model::set_l1(real const val) {
  if (val < 0_r) {
    ERR(L"L1 regularization (%f) less than zero.", val);
    return false;
  } else if (val > 1_r) {
    ERR(L"L1 regularization (%f) bigger than one.", val);
    return false;
  }

  if (this->regularization__l1 != val) {
    bool const use_regularization(this->Use__Regularization_Parameter());

    this->regularization__l1 = val;
...
#ifdef COMPILE_CUDA
    if (this->is_cu_initialized) this->cumodel->set_l1(val);
#endif
  }
