...
namespace DL::v1 {
real Model::get_srip(void) const { return this->regularization__srip; }

bool Model::set_srip(real const val) {
  if (val < 0_r) {
    ERR(L"Srip regularization (%f) less than zero.", val);
    return false;
  } else if (val > 1_r) {
    ERR(L"Srip regularization (%f) bigger than one.", val);
    return false;
  }

  if (this->regularization__srip != val) {
    bool const use_regularization(this->Use__Regularization_Parameter());

    this->regularization__srip = val;
...
#ifdef COMPILE_CUDA
    if (this->is_cu_initialized) this->cumodel->set_srip(val);
#endif
  }
...
void Model::Update_Derivative_Weight__Regularization__SRIP(
  ...
  if (this->use_mp && this->is_mp_initialized)
    this->Update_Derivative_Weight__Regularization__SRIP__OpenMP(str, end,
                                                               batch_size);
  else
    this->Update_Derivative_Weight__Regularization__SRIP__Loop(str, end,
                                                             batch_size);
}
