...
// Sigmoid.
#define AF_SIGMOID_real(x)   (1_r / (1_r + Math::op_exp(-x)))
#define AF_SIGMOID_derive(q) (q * (1_r - q))

// Sigmoid symmetric.
#define AF_TANH_real(x)   (2_r / (1_r + Math::op_exp(-2_r * x)) - 1_r)
#define AF_TANH_derive(q) (1_r - q * q)

// Gaussian.
#define AF_GAUSSIAN_real(x)      Math::op_exp(-x* x)
#define AF_GAUSSIAN_derive(x, q) (-2_r * x * q)

// Gaussian symmetric.
#define AF_GAUSSIAN_SYMMETRIC_real(x)      (Math::op_exp(-x * x) * 2_r - 1_r)
#define AF_GAUSSIAN_SYMMETRIC_derive(x, q) (-2_r * x * (q + 1_r))

// Elliot.
#define AF_ELLIOT_real(x) ((x * 0.5_r) / (1_r + Math::op_abs(x)) + 0.5_r)
#define AF_ELLIOT_derive(x) \
  (1_r / (2_r * (1_r + std::abs(x)) * (1_r + std::abs(x))))

// Elliot symmetric.
#define AF_ELLIOT_SYMMETRIC_real(x) (x / (1_r + Math::op_abs(x)))
#define AF_ELLIOT_SYMMETRIC_derive(x) \
  (1_r / ((1_r + std::abs(x)) * (1_r + std::abs(x))))

// Sine.
#define AF_SIN_real(x)   (Math::op_sin(x) * 0.5_r + 0.5_r)
#define AF_SIN_derive(x) (std::cos(x) * 0.5_r)

// Sine symmetric.
#define AF_SIN_SYMMETRIC_real(x)   Math::op_sin(x)
#define AF_SIN_SYMMETRIC_derive(x) std::cos(x)

// Cosine.
#define AF_COS_real(x)   (Math::op_cos(x) * 0.5_r + 0.5_r)
#define AF_COS_derive(x) (-std::sin(x) * 0.5_r)

// Cosine symmetric.
#define AF_COS_SYMMETRIC_real(x)   Math::op_cos(x)
#define AF_COS_SYMMETRIC_derive(x) -std::sin(x)

//  Inverse square root unit.
#define AF_ISRU_real(x, slope)      (x * (1_r / Math::op_sqrt(1_r + slope * x * x)))
#define AF_ISRU_derive(x, q, slope) (std::pow(q / x, 3_r))

//  Inverse square root linear unit.
#define AF_ISRLU_real(x, slope) \
  (x < 0_r ? x * (1_r / Math::op_sqrt(1_r + slope * x * x)) : x)
#define AF_ISRLU_derive(x, q, slope) (x < 0_r ? std::pow(q / x, 3_r) : 1_r)

// Exponential linear unit.
#define AF_ELU_real(x, slope)      (x < 0_r ? slope * (Math::op_exp(x) - 1_r) : x)
...
#define AF_SELU_real(x) \
  (SELU_Scale * (x <= 0_r ? SELU_Alpha * Math::op_exp(x) - SELU_Alpha : x))
...
// Softmax.
#define AF_SOFTMAX_real(x) Math::op_exp(x)
// w.r.t Cross-entropy.
