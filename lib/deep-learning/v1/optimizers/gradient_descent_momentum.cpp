...
namespace DL::v1 {
void Model::Update_Parameter__Gradient_Descent_Momentum__Loop(
  ...
  if (wd != 0_r) {
    for (i = str; i != end; ++i) {
      ...
      // Gradient descent
      parameters[i] -= delta + mask[i] * wd * parameters[i];
...
void Model::Update_Parameter__Gradient_Descent_Momentum__OpenMP(
    size_t const batch_size, size_t const training_size, size_t const str,
    size_t const end) {
