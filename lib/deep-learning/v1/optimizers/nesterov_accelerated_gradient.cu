...
template <typename T>
__global__ void kernel__Update_Parameter_Nesterov_Accelerated_Gradient(
...
template <typename T>
__global__ void kernel_while__Update_Parameter_Nesterov_Accelerated_Gradient(
    size_t const size_received, T const learning_rate_received,
    T const learning_momentum_received,
    T *const ptr_array_partial_derivative_received,
    T *const ptr_array_parameters_received,
...
__device__ void
cuModel::Update_Parameter__Nesterov_Accelerated_Gradient__CUDA(
  ...
  if (wd != 0_r) {
    if (USE_PARALLEL && end_index_received - start_index_received >= warpSize) {
      ...
      if (this->ptr_array_dim3_grid[1].x * this->ptr_array_dim3_block[1].x <
        ...
        kernel_while__Update_Parameter_Nesterov_Accelerated_Gradient<var>
      ...
      //    2: Launching size condition.
               ...
               end_index_received - start_index_received) {
        kernel__Update_Parameter_Nesterov_Accelerated_Gradient<var>
            <<<this->ptr_array_dim3_grid[1], this->ptr_array_dim3_block[1]>>>(
                end_index_received - start_index_received, wd,
                lr, lr_mom,
                mask +
      ...
      for (size_t i(start_index_received); i != end_index_received; ++i) {
        tmp_ptr_array_previous_delta_weights[i] = tmp_delta_weigth =
            lr * tmp_ptr_array_partial_derivative[i] +
            lr_mom * tmp_ptr_array_previous_delta_weights[i];

        tmp_ptr_array_parameters[i] -=
            lr_mom * tmp_delta_weigth -
            lr * tmp_ptr_array_partial_derivative[i] +
            mask[i] *
