...
namespace DL::v1 {
void Model::compute_r_fwp_st(size_t const batch_size,
  ...
  for (i = 0_UZ; i != batch_size; ++i) {
    ...
    for (; unit_it != last_unit; ++unit_it, ++Y_i) {
      ...
      *numerators += y_i_k_mean_diff * q_i_k_mean_diff;
      *denominators_Y += y_i_k_mean_diff * y_i_k_mean_diff;
      *denominators_y += q_i_k_mean_diff * q_i_k_mean_diff;
    }
  }
}

void Model::compute_r_rec_st(size_t const batch_size,
                             real const *const *const Ym) {
  Layer const *const layer(this->Get__Output_Layer());
  ...
  for (i = 0_UZ; i != batch_size; ++i) {
    for (t = this->n_time_delay; t != this->seq_w; ++t) {
      ...
      for (; unit_it != last_unit; ++unit_it, ++Y_i_t) {
        ...
        *numerators += y_i_t_k_mean_diff * q_i_t_k_mean_diff;
        *denominators_Y += y_i_t_k_mean_diff * y_i_t_k_mean_diff;
        *denominators_y += q_i_t_k_mean_diff * q_i_t_k_mean_diff;
      }
    }
  }
}

void Model::compute_r_pre_train_fwp_st(size_t const batch_size) {
  Layer const *const layer(this->Get__Output_Layer());
...
void Model::compute_r_pre_train_rec_st(size_t const batch_size) {
  Layer const *const layer(this->Get__Output_Layer());
