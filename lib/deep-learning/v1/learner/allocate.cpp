...
bool Model::Allocate__Parameter__iRPROP_plus(void) {
    if (this->Allocate__Parameter__iRPROP_minus() == false) {
      ERR(L"An error has been triggered from the "
          L"`Allocate__Parameter__iRPROP_minus()` function.");
      return false;
    }
...
bool Model::Allocate__Parameter__AMSGrad(void) {
    if (this->Allocate__Parameter__Adam() == false) {
      ERR(L"An error has been triggered from the "
          L"`Allocate__Parameter__Adam()` function.");
      return false;
    }
...
bool Model::Allocate__Parameter__Normalization(void) {
    // TODO: Reorganasition of the array.
    // [------Weights-----][----Bias----][----Normalized unit----]. Allocating
    // with the size of each layer. No waste of memory.
    if (this->ptr_array_parameters != nullptr) {
      ...
      size_t const tmp_new_dimension_parameters(
          ...
          2_UZ * this->total_normalized_units);

      if (this->Reallocate__Parameter(tmp_new_dimension_parameters) == false) {
            ERR(L"An error has been triggered from the "
                L"`Reallocate__Parameter(%zu)` function.",
                tmp_new_dimension_parameters);
            return false;
      }

      // Clear shift array.
      VARZERO(this->ptr_array_parameters + this->total_weights_allocated +
                ...
                switch(layer_it->type_layer)
                {
                    ...
                    case LAYER::LSTM:
                        if(static_cast<size_t>(layer_it->ptr_last_block_unit - layer_it->ptr_array_block_units) != 0_UZ)
                        {
                            ...
                            break;
                    default:
                        ERR(L"Type layer (%d | %ls) is not managed"
                            L"in the switch.",
                            layer_it->type_layer,
                            LAYER_NAME[layer_it->type_layer].c_str());
                        return false;
