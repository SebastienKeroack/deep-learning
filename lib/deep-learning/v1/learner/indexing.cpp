...
bool Model::Set__Pre_Training_Level(size_t const pre_training_level_received) {
  if (this->pre_training_level == pre_training_level_received) {
    return true;
  } else if (pre_training_level_received >
             (this->total_layers - 3_UZ) / 2_UZ + 1_UZ) {
    ERR(L"Pre training level (%zu) overflow (%zu).",
        pre_training_level_received, (this->total_layers - 3_UZ) / 2_UZ + 1_UZ);
    return false;
  }
  ...
  if ((tmp_past_pre_training_level == 0_UZ &&
       pre_training_level_received != 0_UZ) ||
      (tmp_past_pre_training_level != 0_UZ &&
       pre_training_level_received == 0_UZ))
  ...
  for (; layer_it != last_layer; ++layer_it) {
    switch (layer_it->type_layer) {
      case LAYER::FULLY_CONNECTED_INDEPENDENTLY_RECURRENT:
        this->Indexing_Regularization__Weights__AF_Ind_Recurrent(1_r, layer_it);
      case LAYER::FULLY_CONNECTED:
      case LAYER::FULLY_CONNECTED_RECURRENT:
        this->Indexing_Regularization__Weights__FC__Forward(1_r, layer_it);
        break;
      case LAYER::LSTM:
        this->Indexing_Regularization__Weights__LSTM(1_r, layer_it);
        break;
      default:
...
void Model::Indexing_Regularization_Parameters__Pre_training(void) {
  if (this->pre_training_level == 0_UZ) {
    ERR(L"The neural network use the pre-training function "
        L"without the mode pre-training activate.");
    return;
  }

  Layer const *const last_layer(this->ptr_last_layer),
      *const tmp_ptr_input_layer(this->ptr_array_layers +
  ...
  for (; layer_it < tmp_ptr_input_layer; ++layer_it) {
    switch (layer_it->type_layer) {
      case LAYER::FULLY_CONNECTED_INDEPENDENTLY_RECURRENT:
        this->Indexing_Regularization__Weights__AF_Ind_Recurrent(0_r, layer_it);
      case LAYER::FULLY_CONNECTED:
      case LAYER::FULLY_CONNECTED_RECURRENT:
        this->Indexing_Regularization__Weights__FC__Forward(0_r, layer_it);
        break;
      case LAYER::LSTM:
        this->Indexing_Regularization__Weights__LSTM(0_r, layer_it);
        break;
      default:
        ERR(L"Layer type (%d | %ls) is not managed in", layer_it->type_layer,
            LAYER_NAME[layer_it->type_layer].c_str());
...
void Model::Indexing_Regularization__Weights__FC__Forward(
  ...
  for (; tmp_ptr_neuron_unit_it != tmp_ptr_last_neuron_unit;
    ...
    tmp_ptr_mask_regularization_it =
        ...
        *tmp_ptr_neuron_unit_it->ptr_first_connection_index;
    tmp_ptr_last_mask_regularization =
        this->ptr_array_mask_regularized_parameters +
        *tmp_ptr_neuron_unit_it->ptr_last_connection_index;

    for (; tmp_ptr_mask_regularization_it != tmp_ptr_last_mask_regularization;
         ++tmp_ptr_mask_regularization_it)
      *tmp_ptr_mask_regularization_it = mask_received;
  }
}

void Model::Indexing_Regularization__Weights__AF_Ind_Recurrent(
    real const mask_received, Layer const *const layer_it) {
  ...
  real *tmp_ptr_array_mask_regularization_it(
      this->ptr_array_mask_regularized_parameters +
      *tmp_ptr_layer_it_ptr_first_AF_Ind_recurrent_unit
           ->ptr_recurrent_connection_index);
  real const *const tmp_ptr_array_mask_regularization_end(
      tmp_ptr_array_mask_regularization_it + tmp_number_units);

  for (; tmp_ptr_array_mask_regularization_it !=
         tmp_ptr_array_mask_regularization_end;
       ++tmp_ptr_array_mask_regularization_it)
    *tmp_ptr_array_mask_regularization_it = mask_received;
}

void Model::Indexing_Regularization__Weights__LSTM(
    real const mask_received, Layer const *const layer_it) {
  ...
  for (; tmp_ptr_block_unit_it != tmp_ptr_last_block_unit;
    ...
    // [0] Cell input.
    for (tmp_ptr_block_ptr_last_cell_unit =
        ...
        tmp_ptr_block_ptr_cell_unit_it =
         ...
         ++tmp_ptr_block_ptr_cell_unit_it) {
      //    [1] Input, cell input.
      tmp_ptr_array_cell_input_regularized_connections =
          this->ptr_array_mask_regularized_parameters +
          tmp_ptr_block_ptr_cell_unit_it
      ...
      for (tmp_connection_index = 0_UZ;
        ...
        tmp_ptr_array_cell_input_regularized_connections[tmp_connection_index] =
            mask_received;
    ...
    for (tmp_connection_index = 0_UZ;
         tmp_connection_index != tmp_number_feedforward_connections;
         ++tmp_connection_index) {
      tmp_ptr_array_input_gate_regularized_connections[tmp_connection_index] =
          mask_received;
      tmp_ptr_array_forget_gate_regularized_connections[tmp_connection_index] =
          mask_received;
      tmp_ptr_array_output_gate_regularized_connections[tmp_connection_index] =
          mask_received;
    }
    // |END| Input, gates. |END|
...
void Model::Indexing_Regularization__Bias(real const mask_received,
