...
void Model::compute_error_fwp_mp(size_t const batch_size,
  ...
  size_t k, k_target(n_out), k_predict(0_UZ), i_k;
...
#pragma omp for schedule(static)
  for (i = 0; i < batch_size_; ++i) {
    ...
    i_k = static_cast<size_t>(i) * n_out;
...
void Model::compute_error_rec_mp(size_t const batch_size,
  ...
  size_t const n_out(
      static_cast<size_t>(layer->ptr_last_AF_unit - layer->ptr_array_AF_units));
  size_t t, k, k_target(n_out), k_predict(0_UZ), i_t_k;
...
#pragma omp for schedule(static)
  for (i = 0; i < batch_size_; ++i) {
    ...
    for (t = this->n_time_delay; t != this->seq_w; ++t) {
      ...
      i_t_k = this->batch_size * n_out * t + static_cast<size_t>(i) * n_out;
...
#pragma omp for schedule(static)
  for (i = 0; i < batch_size_; ++i) {
    ...
    Y_i = this->get_out(
        this->ptr_array_layers + (this->pre_training_level - 1_UZ),
