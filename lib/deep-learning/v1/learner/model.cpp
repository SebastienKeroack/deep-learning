...
bool Layer::Compare__Dimensions(Layer const &ref_source_Layer_received) const {
  return (
...
bool Neural_Network_Initializer::Build__Layer__FC(
  ...
  if (ref_Layer_Parameters_received.type_layer == LAYER::FULLY_CONNECTED ||
    ...
    ref_Layer_Parameters_received.unit_parameters[0] =
        parse_discrete(1_UZ, L"Neuron(s): ");
  } else {
    ERR(L"Layer type (%d | %ls) is not a fully connected layer.",
        ref_Layer_Parameters_received.type_layer,
...
bool Neural_Network_Initializer::Build__Layer__LSTM(
  ...
  if (ref_Layer_Parameters_received.type_layer == LAYER::LSTM) {
    ref_Layer_Parameters_received.use_bidirectional = accept(L"Bidirectional?");

    ref_Layer_Parameters_received.unit_parameters[0] =
        parse_discrete(1_UZ, L"Block(s): ");

    ref_Layer_Parameters_received.unit_parameters[1] =
        parse_discrete(1_UZ, L"Cells(s) per block: ");
  } else {
    ERR(L"Layer type (%d | %ls) is not a LSTM layer.",
        ref_Layer_Parameters_received.type_layer,
        LAYER_NAME[ref_Layer_Parameters_received.type_layer].c_str());
    return false;
  }
...
bool Neural_Network_Initializer::Build__Layer__Residual(void) {
  ...
  tmp_number_residual_units =
      parse_discrete(1_UZ, L"Number residual unit(s): ");

  INFO(L"");
  INFO(L"Block width.");
  INFO(L"Range[2, inf].");
  INFO(L"default=2.0.");
  tmp_block_depth = parse_discrete(2_UZ, L"Block width: ");
  ...
  if (accept(L"Use widening factor, alpha?")) {
    ...
    do {
      ...
      for (int i(1); i != LAYER::LENGTH; ++i)
        INFO(L"[%d]: %ls.", i, LAYER_NAME[static_cast<LAYER::TYPE>(i)].c_str());
      INFO(L"default=%ls.", LAYER_NAME[LAYER::FULLY_CONNECTED].c_str());

      tmp_widening_Layer_Parameters.type_layer = static_cast<LAYER::TYPE>(
          parse_discrete(1, LAYER::LENGTH - 1, L"Residual unit, type: "));
      ...
      switch (tmp_widening_Layer_Parameters.type_layer) {
        ...
        case LAYER::LSTM:
          if (this->Build__Layer__LSTM(tmp_widening_Layer_Parameters) ==
              false) {
            ERR(L"An error has been triggered from the "
                L"`Build__Layer__LSTM()` function.");
            return false;
          }
          break;
        default:
          ERR(L"Layer type (%d | %ls) is not managed in the "
              "switch.",
      ...
      tmp_widening_factor_alpha = static_cast<double>(
          parse_discrete(-static_cast<long long int>(
      ...
      if (tmp_widening_factor_units[1] != 0.0) {
        ...
        tmp_widening_factor_alpha = static_cast<double>(parse_discrete(
    ...
    for (tmp_residual_unit_index = 0_UZ;
      ...
      for (tmp_layer_index = 0_UZ; tmp_layer_index != tmp_block_depth;
        ...
        for (int i(1); i != LAYER::LENGTH; ++i)
          INFO(L"[%d]: %ls.", i,
               LAYER_NAME[static_cast<LAYER::TYPE>(i)].c_str());
        INFO(L"default=%ls.", LAYER_NAME[LAYER::FULLY_CONNECTED].c_str());

        tmp_Layer_Parameters.type_layer =
            static_cast<LAYER::TYPE>(parse_discrete(
                1, LAYER::LENGTH - 1,
                (L"Residual[" + std::to_wstring(tmp_residual_unit_index) +
                 L"], layer[" + std::to_wstring(tmp_layer_index) + L"], type: ")
                    .c_str()));
        ...
        switch (tmp_Layer_Parameters.type_layer) {
          ...
          case LAYER::MAX_POOLING:
            if (this->Build__Layer__Pooling(tmp_Layer_Parameters) == false) {
              ERR(L"An error has been triggered from the "
                  L"`Build__Layer__Pooling()` function.");
              return false;
            }
            break;
          case LAYER::FULLY_CONNECTED:
...
bool Neural_Network_Initializer::While__Push_Back__Layer(void) {
  ...
  while (
      this->vector_layers_parameters.size() < 2_UZ ||
      accept((L"Add another layer (" + std::to_wstring(tmp_layer_index) + L")?")
                 .c_str())) {
    INFO(L"");
    INFO(L"Layer type.");
    for (int i(1); i != LAYER::LENGTH; ++i)
    ...
    tmp_Layer_Parameters.type_layer = static_cast<LAYER::TYPE>(parse_discrete(
        1, LAYER::LENGTH - 1,
    ...
    switch (tmp_Layer_Parameters.type_layer) {
      ...
      case LAYER::RESIDUAL:
        if (tmp_layer_index <= 1_UZ) {
          ERR(L"Layer type (%d | %ls) is not managed as the first "
              L"hidden/input layer.",
              tmp_Layer_Parameters.type_layer,
              LAYER_NAME[tmp_Layer_Parameters.type_layer].c_str());
          continue;
        } else if (this->type_neural_network == MODEL::AUTOENCODER) {
          ERR(L"The autoencoder network can not use residual layer.");
          continue;
        }

        if (this->Build__Layer__Residual() == false) {
          ERR(L"An error has been triggered from the "
              L"`Build_Residual_Block()` function.");
          return false;
        }
        break;
      default:
...
bool Neural_Network_Initializer::Input_Initialize(void) {
  ...
  for (int i(1); i != MODEL::LENGTH; ++i)
    INFO(L"[%d]: %ls.", i, MODEL_NAME[static_cast<MODEL::TYPE>(i)].c_str());
  INFO(L"default=%ls.", MODEL_NAME[MODEL::FEEDFORWARD].c_str());

  this->type_neural_network =
      static_cast<MODEL::TYPE>(parse_discrete(1, MODEL::LENGTH - 1, L"Type: "));

  INFO(L"");
  this->seq_w = parse_discrete(1_UZ, L"Recurrent depth: ");

  if (this->seq_w > 1_UZ) {
    INFO(L"");
    INFO(L"Time delays.");
    INFO(L"Range[0, %zu].", this->seq_w - 1_UZ);
    this->n_time_delay =
        parse_discrete(0_UZ, this->seq_w - 1_UZ, L"Time delays: ");
  }
  ...
  tmp_Layer_Parameters.unit_parameters[0] =
      parse_discrete(1_UZ, L"Number of inputs: ");
  this->vector_layers_parameters.push_back(tmp_Layer_Parameters);
  ...
  switch (this->type_neural_network) {
    case MODEL::AUTOENCODER:
      if (this->While__Push_Back__Layer() == false) {
        ERR(L"An error has been triggered from the "
            L"`While__Push_Back__Layer()` function.");
        return false;
      }
    ...
    default:
      ...
      tmp_Layer_Parameters.type_layer = LAYER::FULLY_CONNECTED;
      tmp_Layer_Parameters.unit_parameters[0] =
          parse_discrete(1_UZ, L"Number of output(s): ");
      this->vector_layers_parameters.push_back(tmp_Layer_Parameters);
      break;
  }
...
bool Neural_Network_Initializer::Template_Initialize(void) {
  ...
  tmp_Layer_Parameters.type_layer =
      LAYER::FULLY_CONNECTED_INDEPENDENTLY_RECURRENT;
  tmp_Layer_Parameters.unit_parameters[0] =
      parse_discrete(1_UZ, L"First hidden layer, number units: ");
  this->vector_layers_parameters.push_back(tmp_Layer_Parameters);

  INFO(L"Layer[%zu]: Type: %ls.", this->vector_layers_parameters.size() - 1_UZ,
       LAYER_NAME[tmp_Layer_Parameters.type_layer].c_str());
  ...
  INFO(L"Range[1, inf].");
  tmp_number_residual_units = parse_discrete(1_UZ, L"Number residual units: ");

  INFO(L"");
  INFO(L"Residual block, depth.");
  INFO(L"Range[2, inf].");
  INFO(L"default=3.");
  tmp_block_depth = parse_discrete(2_UZ, L"Block depth: ");
  ...
  if ((tmp_use_pooling = accept(L"Do you want to use pooling?"))) {
    ...
    INFO(L"Range[1, %zu].", tmp_number_residual_units);
    INFO(L"default=%zu.",
         static_cast<size_t>(
             ceil(static_cast<double>(tmp_number_residual_units) / 3.0)));
    tmp_pooling_layer_mod = parse_discrete(1_UZ, L"Pooling layer, frequency: ");
  } else { tmp_pooling_layer_mod = 1_UZ; }
  ...
  INFO(L"Range[-%zu, inf] 0=Fixed.",
       tmp_widening_Layer_Parameters.unit_parameters[0] - 1_UZ);
  tmp_widening_factor_alpha = static_cast<double>(parse_discrete<long long int>(
  ...
  for (tmp_residual_unit_index = 0_UZ;
    ...
    if (tmp_use_pooling &&
      ...
      INFO(L"Layer[%zu]: Ceil mode: %ls.",
           this->vector_layers_parameters.size() - 1_UZ,
           tmp_pooling_layer_Parameters.unit_parameters[4] != 0_UZ ? L"true"
                                                                   : L"false");
    }
...
Model *Neural_Network_Initializer::Output_Initialize(
  ...
  if (this->vector_layers_parameters.empty()) {
    ERR(L"The vector `layers parameters` is empty.");
    return nullptr;
  }
  ...
  if (model->compile(this->vector_layers_parameters.size(), this->seq_w,
                     this->type_neural_network,
                     this->vector_layers_parameters.data(),
                     allowable_memory) == false) {
    ERR(L"An error has been triggered from the "
        L"`compile(%zu, ptr, %zu)` function.",
        this->vector_layers_parameters.size(), allowable_memory);
    SAFE_DELETE(model);
  } else if (model->set_seq_w(this->n_time_delay) == false) {
    ERR(L"An error has been triggered from the "
        L"`set_seq_w(%zu)` function.",
        this->n_time_delay);
    SAFE_DELETE(model);
  }
...
bool Activation_Function_Initializer::Input_Initialize(
    size_t const n_layers, MODEL::TYPE const type_network_received) {
  size_t tmp_layer_index;

  if (this->Allocate__Layers_Activation_Function(n_layers) == false) {
    ERR(L"An error has been triggered from the "
        L"`Allocate__Layers_Activation_Function(%zu)` function.",
        n_layers);
    return false;
  }
  ...
  for (int i(1); i != ACTIVATION::LENGTH; ++i)
    INFO(L"[%d]: %ls.", i,
         ACTIVATION_NAME[static_cast<ACTIVATION::TYPE>(i)].c_str());
  INFO(L"default=%ls.", ACTIVATION_NAME[ACTIVATION::LEAKY_RELU].c_str());
    ...
    case MODEL::AUTOENCODER:
      ...
      for (tmp_layer_index = 1_UZ;
        ...
        this->ptr_array_type_layers_activation_function[tmp_layer_index] =
            static_cast<ACTIVATION::TYPE>(parse_discrete(
      ...
      this->ptr_array_type_layers_activation_function[tmp_layer_index] =
          static_cast<ACTIVATION::TYPE>(
              parse_discrete(1, ACTIVATION::LENGTH - 1,
                             L"Coded layer, activation function: "));

      // Decoded layer(s).
      for (++tmp_layer_index; tmp_layer_index != n_layers; ++tmp_layer_index)
        this->ptr_array_type_layers_activation_function[tmp_layer_index] =
            this->ptr_array_type_layers_activation_function
                [n_layers - tmp_layer_index - 1_UZ];  // Subtract coded layer.
      break;
    default:
      // Hidden layer(s).
      for (tmp_layer_index = 1_UZ; tmp_layer_index != n_layers - 1_UZ;
           ++tmp_layer_index)
        this->ptr_array_type_layers_activation_function[tmp_layer_index] =
            static_cast<ACTIVATION::TYPE>(parse_discrete(
                1, ACTIVATION::LENGTH - 1,
      ...
      INFO(L"");
      INFO(L"Output layer:");
      INFO(L"default=%ls.", ACTIVATION_NAME[ACTIVATION::SIGMOID].c_str());

      this->ptr_array_type_layers_activation_function[n_layers - 1_UZ] =
          static_cast<ACTIVATION::TYPE>(
              parse_discrete(1, ACTIVATION::LENGTH - 1,
...
bool Activation_Function_Initializer::Output_Initialize(
  ...
  for (tmp_layer_index = 0_UZ; tmp_layer_index != tmp_number_layers;
    ...
    if (ptr_Neural_Network_received->set_layer_activation_function(
        ...
        false) {
      ERR(L"An error has been triggered from the "
          L"`set_layer_activation_function(%zu, %d)` function.",
          tmp_layer_index,
          this->ptr_array_type_layers_activation_function[tmp_layer_index]);
      return false;
    }
...
bool Loss_Function_Initializer::Input_Initialize(void) {
  ...
  for (int i(1); i != LOSS_FN::LENGTH; ++i)
    INFO(L"[%d]: %ls.", i, LOSS_FN_NAME[static_cast<LOSS_FN::TYPE>(i)].c_str());
  INFO(L"default=%ls.", LOSS_FN_NAME[LOSS_FN::RMSE].c_str());

  this->type_loss_function = static_cast<LOSS_FN::TYPE>(
      parse_discrete(1, LOSS_FN::LENGTH - 1, L"Choose: "));

  if (LOSS_FN::BIT == this->type_loss_function) {
    INFO(L"Loss function, BIT.");
    INFO(L"Range[0.0, 1.0].");
    INFO(L"default=0.5.");

    this->bit_fail_limit = parse_real(0_r, 1_r, L"Bit fail limit: ");
  }
...
bool Accuracy_Function_Initializer::Input_Initialize(void) {
  ...
  for (int i(1); i != ACCU_FN::LENGTH; ++i)
    INFO(L"[%d]: %ls.", i, ACC_FN_NAME[static_cast<ACCU_FN::TYPE>(i)].c_str());
  INFO(L"default=%ls.", ACC_FN_NAME[ACCU_FN::DISTANCE].c_str());

  this->type_accuracy_function = static_cast<ACCU_FN::TYPE>(
      parse_discrete(1, ACCU_FN::LENGTH - 1, L"Choose: "));
...
void Accuracy_Function_Initializer::Output_Initialize(
    Model *const ptr_Neural_Network_received) const {
  ptr_Neural_Network_received->set_accu_fn(this->type_accuracy_function);
}

bool Optimizer_Function_Initializer::Input_Initialize(void) {
  INFO(L"");
  INFO(L"Optimizer functions:");
  for (int i(1); i != OPTIMIZER::LENGTH; ++i) {
    INFO(L"[%d]: %ls.", i,
         OPTIMIZER_NAME[static_cast<OPTIMIZER::TYPE>(i)].c_str());
  }
  INFO(L"default=%ls.", OPTIMIZER_NAME[OPTIMIZER::AMSGRAD].c_str());

  this->type_optimizer_function = static_cast<OPTIMIZER::TYPE>(
      parse_discrete(1, OPTIMIZER::LENGTH - 1, L"Choose: "));
  ...
  switch (this->type_optimizer_function) {
    case OPTIMIZER::GD:
      ...
      INFO(L"Range[0.0, inf].");
      INFO(L"default=0.9");
      this->values[1] = parse_real(0_r, L"Learning momentum: ");
    ...
    case OPTIMIZER::IRPROP_PLUS:
      ...
      INFO(L"Range[0.0, inf].");
      INFO(L"default=1.2.");
      this->values[0] = parse_real(0_r, L"Increase factor: ");

      INFO(L"");
      INFO(L"Decrease factor.");
      INFO(L"Range[0.0, inf].");
      INFO(L"default=0.5.");
      this->values[1] = parse_real(0_r, L"Decrease factor: ");
    ...
    case OPTIMIZER::AMSGRAD:
      ...
      INFO(L"default=0.9.");
      this->values[1] = parse_real(0_r, 1_r - 1e-7_r, L"Beta1: ");

      INFO(L"");
      INFO(L"Beta2.");
      INFO(L"Range[0.0, 0.99...9].");
      INFO(L"default=0.999.");
      this->values[2] = parse_real(0_r, 1_r - 1e-7_r, L"Beta2: ");
    ...
    case OPTIMIZER::NOSADAM:
      ...
      INFO(L"default=0.1.");
      this->values[5] = parse_real(1e-7_r, L"Gamma: ");
      break;
    ...
    case OPTIMIZER::AMSBOUND:
      ...
      INFO(L"Range[0.0, inf].");
      INFO(L"default=0.1.");
      this->values[1] = parse_real(this->values[0], L"Learning rate, final: ");

      INFO(L"");
      INFO(L"Beta1.");
      INFO(L"Range[0.0, 0.99...9].");
      INFO(L"default=0.9.");
      this->values[2] = parse_real(0_r, 1_r - 1e-7_r, L"Beta1: ");

      INFO(L"");
      INFO(L"Beta2.");
      INFO(L"Range[0.0, 0.99...9].");
      INFO(L"default=0.999.");
      this->values[3] = parse_real(0_r, 1_r - 1e-7_r, L"Beta2: ");
      ...
      INFO(L"Range[0.0, 0.99...9].");
      INFO(L"default=1e-3.");
      this->values[6] = parse_real(0_r, 1_r - 1e-7_r, L"Gamma: ");
      break;
    default:
      ERR(L"Type optimizer function (%d | %ls) is not managed in "
          L"the switch.",
  ...
  if (this->type_optimizer_function != OPTIMIZER::IRPROP_PLUS &&
    ...
    INFO(L"default=1e-5.");
    this->weight_decay = parse_real(0_r, 1_r, L"Weight decay: ");
  }
...
bool Optimizer_Function_Initializer::Output_Initialize(
    Model *const ptr_Neural_Network_received) const {
  ptr_Neural_Network_received->set_optimizer(this->type_optimizer_function);
  ...
  switch (this->type_optimizer_function) {
    case OPTIMIZER::GD:
      ...
      if (ptr_Neural_Network_received->learning_momentum != 0_r &&
        ...
        ERR(L"An error has been triggered from the "
            L"`Allocate__Parameter__Gradient_Descent()` function.");
        return false;
      }
  ...
  if (ptr_Neural_Network_received->set_weight_decay(this->weight_decay) ==
      false) {
    ERR(L"An error has been triggered from the "
...
void Warm_Restarts_Initializer::Input_Initialize(void) {
  ...
  if (this->use_warm_restarts) {
    ...
    this->warm_restarts_decay_learning_rate =
        parse_real(1e-5_r, 1_r, L"Learning rate, decay: ");
    ...
    INFO(L"Range[0.0, %f].", this->warm_restarts_maximum_learning_rate);
    INFO(L"default=0.");
    this->warm_restarts_minimum_learning_rate =
        parse_real(0_r, this->warm_restarts_maximum_learning_rate,
    ...
    this->warm_restarts_initial_T_i =
        static_cast<real>(parse_discrete(0_UZ, L"Initial Ti: "));
    ...
    this->warm_restarts_multiplier =
        static_cast<real>(parse_discrete(0_UZ, L"Warm restarts multiplier: "));
  }
}
...
bool Weights_Initializer::Input_Initialize(void) {
  ...
  for (int i(1); i != INITIALIZER::LENGTH; ++i)
    INFO(L"[%d]: %ls.", i,
         INITIALIZER_NAME[static_cast<INITIALIZER::TYPE>(i)].c_str());
  INFO(L"default=%ls.", INITIALIZER_NAME[INITIALIZER::ORTHOGONAL].c_str());

  this->type_weights_initializer = static_cast<INITIALIZER::TYPE>(
      parse_discrete(1, INITIALIZER::LENGTH - 1, L"Type: "));
  ...
  switch (this->type_weights_initializer) {
    ...
    case INITIALIZER::ORTHOGONAL:
      ...
      this->initial_bias = parse_real(0_r, 1_r, L"Initial bias: ");
      break;
    case INITIALIZER::LSUV:
      ...
      this->values[0] =
          static_cast<real>(parse_discrete(0_UZ, L"Maximum number trials: "));
      ...
      this->values[1] =
          static_cast<real>(parse_discrete(1_UZ, L"Maximum batch size: "));
      ...
      this->values[2] = parse_real(0_r, L"Variance target: ");
      ...
      this->values[3] = parse_real(0_r, L"Variance tolerance: ");
      break;
    case INITIALIZER::UNIFORM:
      ...
      this->values[0] = parse_real(-1_r, 1_r, L"Lower bound: ");

      INFO(L"");
      INFO(L"Upper bound:");
      INFO(L"Range[%f, 1.0].", this->values[0]);
      INFO(L"default=1.0.");

      this->values[1] = parse_real(this->values[0], 1_r, L"Upper bound: ");
      break;
    default:
      ERR(L"Type weights initializer (%d | %ls) is not managed in "
          "the switch.",
  ...
  switch (this->type_weights_initializer) {
    ...
    case INITIALIZER::LSUV:
      if (ptr_Neural_Network_received->Initialize__LSUV(
        ...
        ERR(L"An error has been triggered from the "
            L"`Initialize__LSUV(%zu, %zu, %f, %f, %f)` function.",
            static_cast<size_t>(this->values[0]),
            static_cast<size_t>(this->values[1]), this->initial_bias,
...
bool Dropout_Initializer::Input_Initialize(
  ...
  size_t const end(type_network_received == MODEL::AUTOENCODER
                       ? n_layers / 2_UZ + 1_UZ
                       : n_layers - 1_UZ);
  size_t tmp_option;
  ...
  {
    ERR(L"An error has been triggered from the "
        L"`Allocate__Layers_Using_Dropout(%zu)` function.",
        n_layers - 1_UZ);
    return false;
  }
  ...
  while (true) {
    ...
    INFO(L"[0]: Input layer: (%f, %f), %ls.",
         ...
         LAYER_DROPOUT_NAME[this->ptr_array_layers_type_dropout[0]].c_str());
    for (size_t i(1_UZ); i != end; ++i)
      INFO(L"[%zu]: Hidden layer[%zu]: (%f, %f), %ls.", i, i - 1_UZ,
           this->ptr_array_layers_dropout_array_values[i][0],
           this->ptr_array_layers_dropout_array_values[i][1],
           LAYER_DROPOUT_NAME[this->ptr_array_layers_type_dropout[i]].c_str());
    INFO(L"[%zu]: Quit.", end);

    tmp_option = parse_discrete(0_UZ, end, L"Option: ");
    ...
    if (tmp_option < end) {
      ...
      for (int i(0); i != LAYER_DROPOUT::LENGTH; ++i)
        INFO(L"[%d]: %ls.", i,
             LAYER_DROPOUT_NAME[static_cast<LAYER_DROPOUT::TYPE>(i)].c_str());
      INFO(L"default=%ls.",
           LAYER_DROPOUT_NAME[LAYER_DROPOUT::BERNOULLI].c_str());

      switch ((this->ptr_array_layers_type_dropout[tmp_option] =
                   static_cast<LAYER_DROPOUT::TYPE>(parse_discrete(
                       0, LAYER_DROPOUT::LENGTH - 1,
        ...
        case LAYER_DROPOUT::ALPHA:
          ...
          if (this->ptr_array_layers_dropout_array_values[tmp_option][0] != 0_r)
        ...
        case LAYER_DROPOUT::BERNOULLI:
          ...
          this->ptr_array_layers_dropout_array_values[tmp_option][0] =
              parse_real(0_r, 1_r,
                         (tmp_layer_name + L" layer, retention probability: ")
                             .c_str());

          this->ptr_array_layers_dropout_array_values[tmp_option][1] = 0_r;

          if (this->ptr_array_layers_dropout_array_values[tmp_option][0] != 1_r)
            tmp_use_dropout = true;
          break;
        ...
        case LAYER_DROPOUT::ZONEOUT:
          ...
          this->ptr_array_layers_dropout_array_values[tmp_option]
                                                     [0] = parse_real(
              0_r, 1_r,
              (tmp_layer_name + L" layer, zoneout cell probability: ").c_str());
          ...
          break;
        default:
          ERR(L"Type dropout layer (%d | %ls) is not managed in "
              "the switch.",
      ...
      if (MODEL::AUTOENCODER == type_network_received &&
          LAYER_DROPOUT::NONE !=
              this->ptr_array_layers_type_dropout[tmp_option] &&
          0_UZ != tmp_option)
        this->ptr_array_layers_use_coded_dropout[tmp_option] =
            accept(L"Pre-training: Use dropout inside the coded layer?");
    } else if (tmp_option == end) {
      break;
    } else {
      ERR(L"An error has been triggered from the "
...
bool Dropout_Initializer::Output_Initialize(
  ...
  if (this->ptr_array_layers_dropout_array_values != nullptr &&
    ...
    for (size_t i(0_UZ); i != tmp_number_layers; ++i) {
      if (ptr_Neural_Network_received->set_dropout(
              i, this->ptr_array_layers_type_dropout[i],
              this->ptr_array_layers_dropout_array_values[i]) == false) {
        ERR(L"An error has been triggered from the "
            "\"set_dropout(%zu, %u, %f, %f)\" function.",
            i, this->ptr_array_layers_type_dropout[i],
            this->ptr_array_layers_dropout_array_values[i][0],
            this->ptr_array_layers_dropout_array_values[i][1]);
        return false;
      ...
      ptr_Neural_Network_received->ptr_array_layers[i].use_coded_dropout =
          this->ptr_array_layers_use_coded_dropout[i];
    }
  }
...
bool Dropout_Initializer::Allocate__Layers_Using_Dropout(
    size_t const n_layers) {
  bool *tmp_ptr_array_layers_use_coded_dropout(new bool[n_layers]);
  Mem::fill(tmp_ptr_array_layers_use_coded_dropout,
  ...
  Mem::fill_null<real *>(tmp_ptr_array_layers_dropout_array_values,
  ...
  for (size_t i(0_UZ); i != n_layers; ++i)
    this->ptr_array_layers_dropout_array_values[i] =
        tmp_ptr_array_values + i * 2_UZ;
  // |END| Dropout value. |END|

  LAYER_DROPOUT::TYPE *tmp_ptr_array_layers_type_dropout(
      new LAYER_DROPOUT::TYPE[n_layers]);
  Mem::fill(tmp_ptr_array_layers_type_dropout,
            tmp_ptr_array_layers_type_dropout + n_layers, LAYER_DROPOUT::NONE);
...
bool Normalization_Initializer::Input_Initialize(
  ...
  size_t const tmp_option_end(type_network_received == MODEL::AUTOENCODER
                                  ? n_layers / 2_UZ + 1_UZ
                                  : n_layers - 1_UZ);
  size_t tmp_option, layer_idx;

  if (this->Allocate__Layers_Using_Normalization(n_layers - 1_UZ) ==
      false)  // Subtract output layer.
  {
    ERR(L"An error has been triggered from the "
        L"`Allocate__Layers_Using_Normalization(%zu)` function.",
        n_layers - 1_UZ);
    return false;
  }
  ...
  while (true) {
    ...
    for (size_t i(1_UZ); i != tmp_option_end; ++i)
      INFO(L"[%zu] Hidden layer[%zu]: %ls, %ls.", i - 1_UZ, i - 1_UZ,
           LAYER_NORM_NAME[this->ptr_array_layers_using_normalization[i]]
               .c_str(),
           this->ptr_array_layers_normalization_before_activation[i]
               ? L"true"
               : L"false");
    INFO(L"[%zu]: Quit.", tmp_option_end - 1_UZ);

    tmp_option =
        parse_discrete(0_UZ, tmp_option_end - 1_UZ, L"Option: ") + 1_UZ;

    if (tmp_option < tmp_option_end) {
      layer_idx = tmp_option;

      INFO(L"");
      INFO(L"Layer normalization:");
      for (int i(0); i != LAYER_NORM::LENGTH; ++i)
        INFO(L"[%d]: %ls.", i,
             LAYER_NORM_NAME[static_cast<LAYER_NORM::TYPE>(i)].c_str());
      INFO(L"default=%ls.",
           LAYER_NORM_NAME[LAYER_NORM::BATCH_RENORMALIZATION].c_str());

      this->ptr_array_layers_using_normalization[layer_idx] =
          static_cast<LAYER_NORM::TYPE>(parse_discrete(
              0, LAYER_NORM::LENGTH - 1,
              (L"Hidden layer " + std::to_wstring(layer_idx) + L", type: ")
                  .c_str()));

      if (this->ptr_array_layers_using_normalization[layer_idx] !=
          LAYER_NORM::NONE)
        this->ptr_array_layers_normalization_before_activation[layer_idx] =
            accept((L"Hidden layer " + std::to_wstring(layer_idx) +
                    L", use normalization before activation?")
                       .c_str());

      switch (this->ptr_array_layers_using_normalization[layer_idx]) {
  ...
  if (tmp_use_batch_normalization || tmp_use_batch_renormalization) {
    ...
    this->normalization_epsilon = parse_real(0_r, L"Epsilon: ");
...
bool Normalization_Initializer::Output_Initialize(
  ...
  if (this->ptr_array_layers_using_normalization != nullptr) {
    ...
    for (size_t i(1_UZ); i != tmp_number_layers; ++i) {
      if (ptr_Neural_Network_received->Set__Layer_Normalization(
              i, this->ptr_array_layers_using_normalization[i]) == false) {
        ERR(L"An error has been triggered from the "
            L"`Set__Layer_Normalization(%zu, %d)` function.",
            i, this->ptr_array_layers_using_normalization[i]);
        return false;
      }

      switch (this->ptr_array_layers_using_normalization[i]) {
        case LAYER_NORM::BATCH_NORMALIZATION:
      ...
      ptr_Neural_Network_received->ptr_array_layers[i]
...
bool Normalization_Initializer::Allocate__Layers_Using_Normalization(
  ...
  this->ptr_array_layers_using_normalization = new LAYER_NORM::TYPE[n_layers]();
...
bool Model::set_cu(bool const use_cuda, size_t const allowable_memory) {
  ...
  if ((this->use_cu == false && use_cuda) ||
    ...
    if (this->Initialize__CUDA(allowable_memory) == false) {
      ERR(L"An error has been triggered from the "
          L"`Initialize__CUDA(%zu)` function.",
          allowable_memory);
      return false;
    }
...
bool Model::Initialize__CUDA(size_t const allowable_memory) {
  ...
  if (this->is_cu_initialized == false) {
    ...
    if (this->cumodel->Copy__Host_To_Device(this, allowable_memory) == false) {
      ERR(L"An error has been triggered from the "
          L"`Copy__Host_To_Device(ptr, %zu)` function.",
          allowable_memory);
...
bool Model::Initialize__CUDA__Thread(Datasets const *const datasets) {
    ...
    Datasets const *const datasets) {
      ...
      tmp_number_examples_max = std::max<size_t>(tmp_number_examples_max,
                                                 tmp_number_examples_training);
      tmp_number_examples_max = std::max<size_t>(
          tmp_number_examples_max, tmp_number_examples_validation);
      tmp_number_examples_max = std::max<size_t>(tmp_number_examples_max,
                                                 tmp_number_examples_testing);

      INFO(L"GPU: Neural network: Update threads size");
      if (this->cumodel->update_mem_thread_size(tmp_number_examples_max) ==
          false) {
        ERR(L"An error has been triggered from the "
      ...
      if (this->cumodel->update_mem_batch_size(tmp_number_examples_max) ==
          false) {
        ERR(L"An error has been triggered from the "
      ...
      INFO(
          L"GPU: Neural network: Setting up limit device runtime "
          L"pending launch count.");
  ...
  ERR(L"`CUDA` functionality was not built. "
    ...
    bool Model::Use__Clip_Gradient(void) const {
      return (this->use_clip_gradient);
    }

    bool Model::Use__Regularization_Parameter(void) const {
      if (this->regularization__l1 != 0_r || this->regularization__l2 != 0_r ||
          this->regularization__srip != 0_r || this->weight_decay != 0_r)
        return true;
      return false;
    }
    ...
    bool Model::Set__Multi_Label(bool const use_multi_label_received) {
      if (this->use_multi_label == use_multi_label_received) {
        ...
        ERR(L"Can not use multi label with only one output.");
        return false;
      }
      ...
      switch (this->type) {
        case MODEL::AUTOENCODER:
          if (use_first_layer_as_input_received == false &&
              this->use_last_layer_as_output ==
                  use_first_layer_as_input_received) {
            ERR(L"Can not use the decoded layer has input. "
                L"The decoded layer is the output.");
            return false;
          }
          ...
          break;
        default:
          ERR(L"Network type (%d | %ls) is not managed in the switch.",
              this->type, MODEL_NAME[this->type].c_str());
          return false;
      }
      ...
      switch (this->type) {
        case MODEL::AUTOENCODER:
          if (use_last_layer_as_output_received == false &&
              this->use_first_layer_as_input ==
                  use_last_layer_as_output_received) {
            ERR(L"Can not use the decoded layer has output. "
                L"The decoded layer is the input.");
            return false;
          }
    ...
    real Model::activation_fn_derivative(
        ACTIVATION::TYPE const type, real const x, real const q, real const y) {
      switch (type) {
        ...
        case ACTIVATION::SOFTMAX:
          ...
          return q * (y - q);
        default:
          ERR(L"Activation function type (%d | %ls) is not managed in the "
              L"switch.",
              type, ACTIVATION_NAME[type].c_str());
          return q;
      }
    ...
    bool Model::set_max_batch_size(size_t const max_batch_size) {
      ...
      if (this->maximum_batch_size != max_batch_size) {
        ...
        if (this->update_mem_batch_size(max_batch_size, true) == false) {
          ERR(L"An error has been triggered from the "
              L"`update_mem_batch_size(%zu, true)` function.",
              max_batch_size);
          return false;
        }
      }
    ...
    void Model::Clear_Outputs(void) {
      ...
      switch (this->type) {
        case MODEL::AUTOENCODER:
          ...
          if (this->use_last_layer_as_output ||
              this->pre_training_level != 0_UZ)
          ...
          // Else it use the coded part as output.
            ...
            tmp_ptr_output_layer = this->ptr_last_layer -
                                   ((this->total_layers - 3_UZ) / 2_UZ + 2_UZ);
          break;
        default:
      ...
      size_t const tmp_number_outputs(
          *tmp_ptr_output_layer->ptr_number_outputs);

      VARZERO(
          tmp_ptr_output_layer->ptr_array_outputs,
          this->batch_size * tmp_number_outputs * this->seq_w * sizeof(var));
    }
    ...
    real Model::get_layer_variance(size_t const layer_index_received,
      ...
      return this->get_layer_variance(
          this->ptr_array_layers + layer_index_received, max_batch_size);
    }

    real Model::get_layer_variance(Layer const *const ptr_layer_received,
                                   size_t const max_batch_size) const {
      if (max_batch_size == 0_UZ) {
        ERR(L"Maximum batch size can not be equal to zero.");
        return 0_r;
      ...
      real const tmp_batch_scaled(
          1_r /
          static_cast<real>(this->seq_w * max_batch_size * tmp_output_size));
      real tmp_output, tmp_mean(0_r), tmp_variance(0_r);

      for (size_t t(0_UZ), i, k; t != this->seq_w; ++t) {
        for (i = 0_UZ; i != max_batch_size; ++i) {
          tmp_ptr_array_outputs = tmp_ptr_layer_ptr_array_outputs +
                                  i * tmp_output_size +
                                  this->batch_size * tmp_output_size * t;

          for (k = 0_UZ; k != tmp_output_size; ++k) {
            tmp_output = cast(tmp_ptr_array_outputs[k]);
    ...
    size_t Layer::get_n_out(void) const { return *this->ptr_number_outputs; }
    ...
    std::wstring Model::Get__Parameters(bool const full_description_received) {
      ...
      tmp_string += CRLF tmp_string += L"Network type: " +
                                       MODEL_NAME[this->type] + L", " +
                                       std::to_wstring(this->type) + CRLF;
      tmp_string +=
          L"Number time prediction(s): " + std::to_wstring(this->seq_w) + CRLF;
      tmp_string += L"Number time delay(s): " +
                    std::to_wstring(this->n_time_delay) + CRLF;
      tmp_string +=
          L"Use the first layer as input: " +
          std::wstring(this->use_first_layer_as_input ? L"true" : L"false") +
          CRLF;
      tmp_string +=
          L"Use the last layer as output: " +
      ...
      if (this->type_optimizer_function == OPTIMIZER::GD ||
        ...
        tmp_string +=
            L"Learning rate: " + to_wstring(this->learning_rate) + CRLF;
        tmp_string +=
            L"Learning momentum: " + to_wstring(this->learning_momentum) + CRLF;
        tmp_string += L"Use Nesterov: " +
                      std::wstring(this->use_nesterov ? L"true" : L"false") +
                      CRLF;
      ...
      if (this->type_optimizer_function == OPTIMIZER::QUICKPROP ||
        ...
        tmp_string += L"Decay: " + to_wstring(this->quickprop_decay) + CRLF;
        tmp_string += L"Mu: " + to_wstring(this->quickprop_mu) + CRLF;
      ...
      if (this->type_optimizer_function == OPTIMIZER::IRPROP_PLUS ||
        ...
        tmp_string += L"Increase factor: " +
                      to_wstring(this->rprop_increase_factor) + CRLF;
        tmp_string += L"Decrease factor: " +
                      to_wstring(this->rprop_decrease_factor) + CRLF;
        tmp_string +=
            L"Delta minimum: " + to_wstring(this->rprop_delta_min) + CRLF;
        tmp_string +=
            L"Delta maximum: " + to_wstring(this->rprop_delta_max) + CRLF;
        tmp_string +=
            L"Delta zero: " + to_wstring(this->rprop_delta_zero) + CRLF;
      ...
      if (this->type_optimizer_function == OPTIMIZER::SARPROP ||
        ...
        tmp_string += L"Weight decay shift: " +
                      to_wstring(this->sarprop_weight_decay_shift) + CRLF;
        tmp_string += L"Step error threshold factor: " +
                      to_wstring(this->sarprop_step_error_threshold_factor) +
                      CRLF;
        tmp_string += L"Step error shift: " +
                      to_wstring(this->sarprop_step_error_shift) + CRLF;
        tmp_string +=
            L"Temperature: " + to_wstring(this->sarprop_temperature) + CRLF;
        tmp_string +=
            L"Epoch(s): " + std::to_wstring(this->sarprop_epoch) + CRLF;
      ...
      if (this->type_optimizer_function == OPTIMIZER::NOSADAM ||
        ...
        tmp_string += L"|===| " +
                      ...
                      L" PARAMETERS |===|" CRLF;
        tmp_string +=
            L"Learning rate: " + to_wstring(this->adam_learning_rate) + CRLF;
        tmp_string += L"Beta1: " + to_wstring(this->adam_beta1) + CRLF;
        tmp_string += L"Beta2: " + to_wstring(this->adam_beta2) + CRLF;
        tmp_string += L"Epsilon: " + to_wstring(this->adam_epsilon) + CRLF;
        tmp_string +=
            L"Bias correction: " +
            std::wstring(this->use_adam_bias_correction ? L"true" : L"false") +
            CRLF;
        tmp_string += L"Gamma: " + to_wstring(this->adam_gamma) + CRLF;
        tmp_string += L"|END| " +
                      to_upper(OPTIMIZER_NAME[this->type_optimizer_function]) +
        ...
        tmp_string += L"Learning rate, final: " +
                      to_wstring(this->learning_rate_final) + CRLF;
        tmp_string += L"Beta1: " + to_wstring(this->adam_beta1) + CRLF;
        tmp_string += L"Beta2: " + to_wstring(this->adam_beta2) + CRLF;
        tmp_string += L"Epsilon: " + to_wstring(this->adam_epsilon) + CRLF;
        tmp_string +=
            ...
            std::wstring(this->use_adam_bias_correction ? L"true" : L"false") +
            CRLF;
        tmp_string += L"Gamma: " + to_wstring(this->learning_gamma) + CRLF;
        tmp_string += L"|END| " +
                      to_upper(OPTIMIZER_NAME[this->type_optimizer_function]) +
                      L" PARAMETERS |END|" CRLF;
        tmp_string += CRLF;
      ...
      tmp_string += L"Use warm restarts: " +
                    std::wstring(this->use_warm_restarts ? L"true" : L"false") +
                    CRLF;
      if (this->use_warm_restarts) {
        tmp_string += L"Learning rate, decay: " +
                      to_wstring(this->warm_restarts_decay_learning_rate) +
                      CRLF;
        tmp_string +=
            L"Maximum learning rate: " +
            to_wstring(this->warm_restarts_maximum_learning_rate) + L" / " +
            to_wstring(this->warm_restarts_initial_maximum_learning_rate) +
            CRLF;
        tmp_string += L"Minimum learning rate: " +
                      to_wstring(this->warm_restarts_minimum_learning_rate) +
                      CRLF;
        tmp_string += L"Ti: " + to_wstring(this->warm_restarts_T_i) + CRLF;
        tmp_string += L"Initial, Ti: " +
                      to_wstring(this->warm_restarts_initial_T_i) + CRLF;
        tmp_string += L"Warm restart multiplier: " +
                      to_wstring(this->warm_restarts_multiplier) + CRLF;
      }
      ...
      tmp_string += L"Loss function: " +
                    LOSS_FN_NAME[this->type_loss_function] + L", " +
                    std::to_wstring(this->type_loss_function) + CRLF;
      tmp_string += L"Accuracy function: " +
                    ACC_FN_NAME[this->type_accuracy_function] + L", " +
                    std::to_wstring(this->type_accuracy_function) + CRLF;
      if (this->type_loss_function == LOSS_FN::BIT ||
          full_description_received) {
        tmp_string += L"Fail-limit: " + to_wstring(this->bit_fail_limit) + CRLF;
      }
      tmp_string +=
          L"Optimizer time step: " +
          std::to_wstring(static_cast<size_t>(this->optimizer_time_step)) +
          CRLF;
      tmp_string +=
          L"Epoch time step: " +
          std::to_wstring(static_cast<size_t>(this->epoch_time_step)) + CRLF;
      tmp_string += L"Pre-training level: " +
                    std::to_wstring(this->pre_training_level) + CRLF;
      tmp_string +=
          L"Use clip gradient: " +
          std::wstring(this->Use__Clip_Gradient() ? L"true" : L"false") + CRLF;
      if (this->Use__Clip_Gradient() || full_description_received) {
        tmp_string +=
            L"clip gradient: " + to_wstring(this->clip_gradient) + CRLF;
      }
      ...
      tmp_string +=
          ...
          std::wstring(this->Use__Dropout__Bernoulli() ? L"true" : L"false") +
          CRLF;
      tmp_string +=
          ...
          CRLF;
      tmp_string +=
          L"Use dropout, gaussian: " +
          std::wstring(this->Use__Dropout__Gaussian() ? L"true" : L"false") +
          CRLF;
      tmp_string +=
          L"Use dropout, shakedrop: " +
          std::wstring(this->Use__Dropout__ShakeDrop() ? L"true" : L"false") +
          CRLF;
      tmp_string +=
          L"Use dropout, uout: " +
          std::wstring(this->Use__Dropout__Uout() ? L"true" : L"false") + CRLF;
      tmp_string +=
          L"Use dropout, zoneout: " +
          std::wstring(this->Use__Dropout__Zoneout() ? L"true" : L"false") +
          CRLF;
      tmp_string += L"Max-norm contraints: " +
                    to_wstring(this->regularization__max_norm_constraints) +
                    CRLF;
      tmp_string +=
          L"L1 regularization: " + to_wstring(this->regularization__l1) + CRLF;
      tmp_string +=
          L"L2 regularization: " + to_wstring(this->regularization__l2) + CRLF;
      tmp_string += L"SRIP regularization: " +
                    to_wstring(this->regularization__srip) + CRLF;
      tmp_string += L"Weight decay: " + to_wstring(this->weight_decay) + CRLF;
      tmp_string +=
          L"Use normalized weight decay: " +
          std::wstring(this->use_normalized_weight_decay ? L"true" : L"false") +
          CRLF;
      tmp_string +=
          L"Use tied parameter: " +
          std::wstring(this->Use__Tied_Parameter() ? L"true" : L"false") + CRLF;
      tmp_string += L"Use k-Sparse: " +
                    std::wstring(this->Use__K_Sparse() ? L"true" : L"false") +
                    CRLF;
      tmp_string +=
      ...
      tmp_string +=
          ...
          std::wstring(this->Use__Batch_Normalization() ? L"true" : L"false") +
          CRLF;
      tmp_string +=
          L"Use batch renormalization: " +
          std::wstring(this->Use__Batch_Renormalization() ? L"true"
                                                          : L"false") +
          CRLF;
      tmp_string += L"momentum average: " +
                    to_wstring(this->normalization_momentum_average) + CRLF;
      tmp_string += L"normalization epsilon: " +
                    to_wstring(this->normalization_epsilon) + CRLF;
      tmp_string +=
          L"r correction maximum: " +
          to_wstring(this->batch_renormalization_r_correction_maximum) + CRLF;
      tmp_string +=
          L"d correction maximum: " +
          to_wstring(this->batch_renormalization_d_correction_maximum) + CRLF;
      tmp_string += L"|===| NORMALIZATION PARAMETERS |===|" CRLF;
      tmp_string += CRLF;

      tmp_string += L"|===| LOSS PARAMETERS |===|" CRLF;
      tmp_string += L"Train: " + to_wstring(this->loss_train) + CRLF;
      tmp_string += L"Valid: " + to_wstring(this->loss_valid) + CRLF;
      tmp_string += L"Testg: " + to_wstring(this->loss_testg) + CRLF;
      tmp_string += L"|END| LOSS PARAMETERS |END|" CRLF;
      tmp_string += CRLF;

      tmp_string += L"|===| ACCURANCY PARAMETERS |===|" CRLF;
      tmp_string += L"Variance: " + to_wstring(this->acc_var) + CRLF;
      tmp_string += L"Train: " + to_wstring(this->acc_train) + CRLF;
      tmp_string += L"Valid: " + to_wstring(this->acc_valid) + CRLF;
      tmp_string += L"Testg: " + to_wstring(this->acc_testg) + CRLF;
      tmp_string += L"|END| ACCURANCY PARAMETERS |END|" CRLF;
      tmp_string += CRLF;

      tmp_string += L"|===| COMPUTATION PARAMETERS |===|" CRLF;
      tmp_string += L"Use CUDA: " +
                    std::wstring(this->use_cu ? L"true" : L"false") + CRLF;
      tmp_string += L"Use OpenMP: " +
                    std::wstring(this->use_mp ? L"true" : L"false") + CRLF;
      tmp_string += L"Maximum threads (percent): " +
                    std::to_wstring(this->pct_threads) + L"%" + CRLF;
      tmp_string +=
          L"Number of threads: " + std::to_wstring(this->number_threads) + CRLF;
      tmp_string += L"Batch size: " + std::to_wstring(this->batch_size) +
                    L" / " + std::to_wstring(this->maximum_batch_size) + CRLF;
      tmp_string +=
          L"Maximum allowable memory: " +
          std::to_wstring(this->maximum_allowable_memory_bytes) + L" bytes | " +
          to_wstring(static_cast<double>(this->maximum_allowable_memory_bytes) /
                         1024.0 / 1024.0,
                     4, std::ios_base::fixed) +
          L" MBs" + CRLF;
      tmp_string +=
          L"Size for one thread: " +
          std::to_wstring(this->Get__Threads_Sizeof(1u)) + L" bytes | " +
          to_wstring(static_cast<double>(this->Get__Threads_Sizeof(1u)) /
                         1024.0 / 1024.0,
                     4, std::ios_base::fixed) +
          L" MBs" + CRLF;
      tmp_string +=
          ...
          to_wstring(static_cast<double>(this->Get__Batch_Sizeof(1u)) / 1024.0 /
                         1024.0,
                     4, std::ios_base::fixed) +
          L" MBs" + CRLF;
      tmp_string +=
          ...
          L" bytes | " +
          to_wstring(static_cast<double>(this->Get__Sizeof()) / 1024.0 / 1024.0,
                     4, std::ios_base::fixed) +
          L" MBs" + CRLF;
      ...
      tmp_string +=
          L"Total layer(s): " + std::to_wstring(this->total_layers) + CRLF;
      tmp_string += L"Total basic unit(s): " +
                    std::to_wstring(this->total_basic_units) + L"/" +
      ...
      tmp_string += L"Total basic indice unit(s): " +
                    ...
                    std::to_wstring(this->total_basic_indice_units_allocated) +
                    CRLF;
      tmp_string += L"Total neuron unit(s): " +
                    std::to_wstring(this->total_neuron_units) + L"/" +
                    std::to_wstring(this->total_neuron_units_allocated) + CRLF;
      tmp_string += L"Total AF unit(s): " +
                    std::to_wstring(this->total_AF_units) + L"/" +
                    std::to_wstring(this->total_AF_units_allocated) + CRLF;
      tmp_string +=
          L"Total AF Ind unit(s): " +
          std::to_wstring(this->total_AF_Ind_recurrent_units) + L"/" +
          std::to_wstring(this->total_AF_Ind_recurrent_units_allocated) + CRLF;
      tmp_string += L"Total normalized unit(s): " +
                    std::to_wstring(this->total_normalized_units) + L"/" +
                    std::to_wstring(this->total_normalized_units_allocated) +
                    CRLF;
      tmp_string += L"Total block unit(s): " +
                    std::to_wstring(this->total_block_units) + L"/" +
                    std::to_wstring(this->total_block_units_allocated) + CRLF;
      tmp_string += L"Total cell unit(s): " +
                    std::to_wstring(this->total_cell_units) + L"/" +
                    std::to_wstring(this->total_cell_units_allocated) + CRLF;
      tmp_string += L"Total parameter(s): " +
                    std::to_wstring(this->total_parameters) + L"/" +
                    std::to_wstring(this->total_parameters_allocated) + CRLF;
      tmp_string += L"Total weight(s): " +
                    std::to_wstring(this->total_weights) + L"/" +
                    std::to_wstring(this->total_weights_allocated) + CRLF;
      tmp_string += L"Total bias(s): " + std::to_wstring(this->total_bias) +
                    L"/" + std::to_wstring(this->total_bias_allocated) + CRLF;
      ...
      tmp_string += L"    Type: " + LAYER_NAME[layer_it->type_layer] + L", " +
                    std::to_wstring(layer_it->type_layer) + CRLF;
      tmp_string += L"    Type activation: " +
                    LAYER_ACTIVATION_NAME[layer_it->type_activation] + L", " +
                    std::to_wstring(layer_it->type_activation) + CRLF;
      ...
      tmp_string += L"      Dropout value[0]: " +
                    to_wstring(layer_it->dropout_values[0]) + CRLF;
      tmp_string += L"      Dropout value[1]: " +
                    to_wstring(layer_it->dropout_values[1]) + CRLF;
      tmp_string += L"      Dropout value[2]: " +
                    to_wstring(layer_it->dropout_values[2]) + CRLF;
      ...
      auto information_layer_norm_fn([self = this](Layer const *const layer_it)
                                         -> std::wstring {
        ...
        if (static_cast<size_t>(layer_it->ptr_last_normalized_unit -
            ...
            12_UZ) {
          // Normalization.
          if (self->Information__Layer__Normalization(tmp_string, layer_it) ==
              false) {
            ERR(L"An error has been triggered from the "
          ...
          tmp_string += L"    Type normalization: " +
                        LAYER_NORM_NAME[layer_it->type_normalization] + L", " +
                        std::to_wstring(layer_it->type_normalization) + CRLF;
          tmp_string +=
              L"      Use layer normalization before activation: " +
              std::wstring(layer_it->use_layer_normalization_before_activation
                               ? L"true"
                               : L"false") +
              CRLF;
          tmp_string += L"      Number normalized unit(s): " +
      ...
      auto information_layer_fc_fn(
          [self = this](
            ...
            if (*ptr_previous_layer_connected_received->ptr_number_outputs <=
              ...
              // Bias parameter(s).
              if (self->Information__Layer__Bias(tmp_string, layer_it) ==
                  false) {
                ERR(L"An error has been triggered from the "
      ...
      auto information_layer_af_fn(
          [self = this](Layer const *const layer_it) -> std::wstring {
            std::wstring tmp_string(L"");

            if (*layer_it->ptr_number_outputs <= 12_UZ) {
              // AF(s).
              if (self->Information__Layer__AF(tmp_string, layer_it) == false) {
                ERR(L"An error has been triggered from the "
      ...
      auto information_layer_af_ind_recurrent_fn(
          [self = this](Layer const *const layer_it) -> std::wstring {
            ...
            if (*layer_it->ptr_number_outputs <= 12_UZ) {
              ...
              tmp_string += L"    Number AF(s) Ind recurrent: " +
                            std::to_wstring(static_cast<size_t>(
        ...
        switch (layer_it->type_layer) {
          ...
          case LAYER::MAX_POOLING:
            tmp_string += L"    Kernel size: " +
                          std::to_wstring(layer_it->pooling_values[0]) + CRLF;
            tmp_string += L"    Stride: " +
                          std::to_wstring(layer_it->pooling_values[1]) + CRLF;
            tmp_string += L"    Padding: " +
                          std::to_wstring(layer_it->pooling_values[2]) + CRLF;
            tmp_string += L"    Dilation: " +
                          std::to_wstring(layer_it->pooling_values[3]) + CRLF;
            tmp_string +=
                L"    Ceil mode: " +
            ...
            tmp_string += L"    Number feature(s): " +
                          std::to_wstring(*layer_it->ptr_number_outputs) + CRLF;
            break;
          case LAYER::FULLY_CONNECTED:
          case LAYER::FULLY_CONNECTED_RECURRENT:
          case LAYER::FULLY_CONNECTED_INDEPENDENTLY_RECURRENT:
            tmp_string += L"    Type activation: " +
                          ...
                          L", " + std::to_wstring(layer_it->type_activation) +
                          CRLF;
            tmp_string += L"    Type dropout: " +
                          LAYER_DROPOUT_NAME[layer_it->type_dropout] + L", " +
                          std::to_wstring(layer_it->type_dropout) + CRLF;
            ...
            tmp_string +=
                ...
                std::wstring(layer_it->use_tied_parameter ? L"true"
                                                          : L"false") +
                CRLF;
            tmp_string += L"    k-Sparsity: " +
                          std::to_wstring(layer_it->k_sparsity) + CRLF;
            tmp_string += L"    Alpha sparsity: " +
                          to_wstring(layer_it->alpha_sparsity) + CRLF;
            tmp_string +=
                L"    Constraint recurrent weight lower bound: " +
                to_wstring(layer_it->constraint_recurrent_weight_lower_bound) +
                CRLF;
            tmp_string +=
                L"    Constraint recurrent weight upper bound: " +
                to_wstring(layer_it->constraint_recurrent_weight_upper_bound) +
                CRLF;
            tmp_string +=
                L"    First connection index: " +
                std::to_wstring(*layer_it->ptr_first_connection_index) + CRLF;
            tmp_string +=
                L"    Last connection index: " +
                std::to_wstring(*layer_it->ptr_last_connection_index) + CRLF;
            tmp_string +=
                L"    First bias connection index: " +
                std::to_wstring(layer_it->first_bias_connection_index) + CRLF;
            tmp_string +=
            ...
            tmp_string += L"    Number feature(s): " +
                          std::to_wstring(*layer_it->ptr_number_outputs) + CRLF;
            tmp_string +=
                information_layer_fc_fn(layer_it, tmp_ptr_previous_layer);
            tmp_string += information_layer_af_fn(layer_it);
            tmp_string += information_layer_af_ind_recurrent_fn(layer_it);
            break;
          case LAYER::LSTM:
            tmp_string += L"    Type activation: " +
                          LAYER_ACTIVATION_NAME[layer_it->type_activation] +
            ...
            tmp_string += L"    Number feature(s): " +
                          std::to_wstring(*layer_it->ptr_number_outputs) + CRLF;
            tmp_string +=
                information_layer_lstm_fn(layer_it, tmp_ptr_previous_layer);
            break;
          case LAYER::RESIDUAL:
    ...
    void Model::Reset__Parameter__Mask_Dropout(
      ...
      for (layer_it = this->ptr_array_layers; layer_it != last_layer;
           ++layer_it) {
            ...
            case LAYER::LSTM:
              if (static_cast<size_t>(layer_it->ptr_last_block_unit -
                ...
                for (tmp_index = 0_UZ; tmp_index != 3_UZ; ++tmp_index) {
                  for (tmp_ptr_cell_unit_it = layer_it->ptr_array_cell_units;
                       tmp_ptr_cell_unit_it != tmp_ptr_last_cell_unit;
                       ++tmp_ptr_cell_unit_it) {
                    ...
                    tmp_ptr_array_ptr_connections
                        [this->total_normalized_units] = tmp_ptr_cell_unit_it;
                    ++tmp_ptr_array_ptr_connections;
                  }
                }
                ...
                for (tmp_index = 0_UZ; tmp_index != 6_UZ; ++tmp_index) {
                  for (tmp_ptr_block_unit_it = layer_it->ptr_array_block_units;
                       tmp_ptr_block_unit_it != tmp_ptr_last_block_unit;
                       ++tmp_ptr_block_unit_it) {
                    ...
                    tmp_ptr_array_ptr_connections
                        [this->total_normalized_units] = tmp_ptr_block_unit_it;
                    ++tmp_ptr_array_ptr_connections;
                  }
                }
              }
    ...
    void Model::Clear__Parameter__Normalized_Unit(void) {
      ...
      for (layer_it = this->ptr_array_layers; layer_it != last_layer;
        ...
        if ((tmp_number_units = static_cast<size_t>(
                 ...
                 layer_it->ptr_array_normalized_units)) != 0_UZ) {
          // clear shift.
          VARZERO(layer_it->ptr_array_normalized_units->normalized_batch_units
                      .ptr_shift,
    ...
    bool Model::initialize_weights(
      ...
      switch (this->_type_weights_initializer) {
        case INITIALIZER::LSUV:
          ...
          break;
        default:
          ERR(L"Type weights initializer (%d | %ls) is not managed in the "
              L"switch.",
    ...
    size_t Model::Get__Batch_Sizeof(size_t batch_size) const {
      ...
      if (batch_size == 0_UZ) batch_size = this->batch_size;
      ...
      if (this->ptr_array_basic_indice_units_indices != nullptr)
        tmp_total_size_t += batch_size * this->seq_w *
                            this->total_basic_indice_units_allocated *
                            sizeof(size_t);
      ...
      if (this->ptr_array_cells_summations_cells_inputs != nullptr)
        tmp_total_size_t +=
            batch_size * this->seq_w * this->total_cell_units * sizeof(var);
      ...
      if (this->ptr_array_blocks_summations_inputs_gates != nullptr)
        tmp_total_size_t +=
            batch_size * this->seq_w * this->total_block_units * sizeof(var);
      ...
      if (this->ptr_array_cells_delta_inputs != nullptr)
        tmp_total_size_t +=
            batch_size * this->seq_w * this->total_cell_units * sizeof(real);
      ...
      if (this->ptr_array_blocks_delta_inputs_gates != nullptr)
        tmp_total_size_t +=
            batch_size * this->seq_w * this->total_block_units * sizeof(real);
    ...
    size_t Model::Get__Threads_Sizeof(size_t number_threads_received) const {
      ...
      if (this->ptr_array_normalized_batch_units_derivatives_means != nullptr)
        tmp_total_size_t += number_threads_received *
                            this->total_normalized_units_allocated *
                            this->seq_w * sizeof(real);
      if (this->ptr_array_normalized_batch_units_derivatives_variances !=
          nullptr)
        tmp_total_size_t += number_threads_received *
                            this->total_normalized_units_allocated *
                            this->seq_w * sizeof(real);
      ...
      if (this->bernoulli != nullptr)
        tmp_total_size_t += number_threads_received * sizeof(Dist::Bernoulli);
      ...
      if (this->ptr_array_Class_Generator_Real_Uout != nullptr)
        tmp_total_size_t += number_threads_received * sizeof(Dist::Real);
      if (this->ptr_array_Class_Generator_Real_Gaussian != nullptr)
        tmp_total_size_t += number_threads_received * sizeof(Dist::Gaussian);
      // |END| Generator. |END|

      return tmp_total_size_t;
    }

    size_t Model::Get__Sizeof(size_t number_threads_received, size_t batch_size)
        const {
      size_t tmp_total_size_t(0_UZ);

      tmp_total_size_t += sizeof(Model);  // this

      tmp_total_size_t += this->Get__Threads_Sizeof(
          number_threads_received == 0_UZ ? this->number_threads
      ...
      if (this->ptr_array_layers_mask_dropout_shakedrop != nullptr)
        tmp_total_size_t +=
            this->total_layers * this->batch_size * this->seq_w * sizeof(bool);
      ...
      if (this->ptr_array_neuron_units_first_forward_connection_index !=
          nullptr)
      ...
      if (this->ptr_array_cell_units != nullptr)
        tmp_total_size_t += this->total_cell_units_allocated * sizeof(CellUnit);
      ...
      if (this->ptr_array_normalized_units != nullptr)
        tmp_total_size_t += this->total_normalized_units_allocated *
                            sizeof(union Normalized_unit);

      if (this->ptr_array_normalized_batch_units_r_corrections != nullptr)
        tmp_total_size_t +=
            this->total_normalized_units_allocated * this->seq_w * sizeof(var);
    ...
    var const *
    Layer::Get__Array_Summations__Cell__Block_Input__Input__Activation(void)
        const {
    ...
    var const *
    Layer::Get__Array_Summations__Cell__Block_Input__Recurrent__Activation(void)
        const {
      switch (this->type_layer) {
        case LAYER::LSTM:
          if (this->Use__Normalization())
            ...
            return this->ptr_array_cell_units
                ->ptr_summation_recurrent_cell_input;
        default:
          return nullptr;
      }
    }

    var const *Layer::Get__Array_Summations__Cell__Cell_State__Activation(void)
        const {
      switch (this->type_layer) {
        case LAYER::LSTM:
          if (this->Use__Normalization())
    ...
    var const *
    Layer::Get__Array_Summations__Block__Input_Gate__Input__Activation(void)
        const {
      switch (this->type_layer) {
        case LAYER::LSTM:
          if (this->Use__Normalization())
            ...
            return this->ptr_array_block_units
                ->ptr_summation_input_inputs_gates;
        default:
          return nullptr;
      }
    }
    ...
    var const *
    Layer::Get__Array_Summations__Block__Input_Gate__Recurrent__Activation(void)
        const {
      switch (this->type_layer) {
        case LAYER::LSTM:
          if (this->Use__Normalization())
            ...
            return this->ptr_array_block_units
                ->ptr_summation_recurrent_inputs_gates;
        default:
          return nullptr;
      }
    }
    ...
    var const *
    Layer::Get__Array_Summations__Block__Forget_Gate__Input__Activation(void)
        const {
      switch (this->type_layer) {
        case LAYER::LSTM:
          if (this->Use__Normalization())
            ...
            return this->ptr_array_block_units
                ->ptr_summation_input_forgets_gates;
        default:
          return nullptr;
      }
    }
      ...
      switch (this->type_layer) {
        case LAYER::LSTM:
          if (this->Use__Normalization())
            ...
            return this->ptr_array_block_units
                ->ptr_summation_recurrent_forgets_gates;
        default:
          return nullptr;
      }
    }
    ...
    var const *
    Layer::Get__Array_Summations__Block__Output_Gate__Input__Activation(void)
        const {
      switch (this->type_layer) {
        case LAYER::LSTM:
          if (this->Use__Normalization())
            ...
            return this->ptr_array_block_units
                ->ptr_summation_input_outputs_gates;
        default:
          return nullptr;
      }
    }
      ...
      switch (this->type_layer) {
        case LAYER::LSTM:
          if (this->Use__Normalization())
            ...
            return this->ptr_array_block_units
                ->ptr_summation_recurrent_outputs_gates;
        default:
          return nullptr;
      }
    }
    ...
    real const *Layer::Get__Array_Deltas__Cell__Block_Input__Recurrent(void)
        const {
      switch (this->type_layer) {
        case LAYER::LSTM:
          if (this->Use__Normalization())
    ...
    real const *Layer::Get__Array_Deltas__Block__Input_Gate__Recurrent(void)
        const {
      switch (this->type_layer) {
        case LAYER::LSTM:
          if (this->Use__Normalization())
    ...
    real const *Layer::Get__Array_Deltas__Block__Forget_Gate__Input(void)
        const {
      switch (this->type_layer) {
        case LAYER::LSTM:
          if (this->Use__Normalization())
    ...
    real const *Layer::Get__Array_Deltas__Block__Forget_Gate__Recurrent(void)
        const {
      switch (this->type_layer) {
        case LAYER::LSTM:
          if (this->Use__Normalization())
    ...
    real const *Layer::Get__Array_Deltas__Block__Output_Gate__Input(void)
        const {
      switch (this->type_layer) {
        case LAYER::LSTM:
          if (this->Use__Normalization())
    ...
    real const *Layer::Get__Array_Deltas__Block__Output_Gate__Recurrent(void)
        const {
      switch (this->type_layer) {
        case LAYER::LSTM:
          if (this->Use__Normalization())
            return this->ptr_array_block_units
                ->ptr_delta_recurrent_outputs_gates;
          else
            return this->ptr_array_block_units->ptr_delta_outputs_gates;
        default:
          return nullptr;
    ...
    void Model::update_weights_mp(size_t const batch_size,
      ...
      switch (this->type_optimizer_function) {
        case OPTIMIZER::GD:
          this->Update_Parameter__Gradient_Descent(
              batch_size, training_size, 0_UZ, this->total_parameters);
          break;
        case OPTIMIZER::IRPROP_PLUS:
        ...
        case OPTIMIZER::ADABOUND:
          this->Update_Parameters__AdaBound(batch_size, training_size, 0_UZ,
                                            this->total_parameters);
          break;
        case OPTIMIZER::ADAM:
          this->Update_Parameters__Adam(batch_size, training_size, 0_UZ,
                                        this->total_parameters);
          break;
        // case OPTIMIZER::ADAMAX:
        ...
        case OPTIMIZER::AMSBOUND:
          this->Update_Parameters__AMSBound(batch_size, training_size, 0_UZ,
                                            this->total_parameters);
          break;
        case OPTIMIZER::AMSGRAD:
          this->Update_Parameters__AMSGrad(batch_size, training_size, 0_UZ,
                                           this->total_parameters);
          break;
        case OPTIMIZER::NOSADAM:
          this->Update_Parameters__NosAdam(batch_size, training_size, 0_UZ,
                                           this->total_parameters);
          break;
        default:
          ERR(L"Optimizer type (%d | %ls) is not managed in the switch.",
    ...
    Layer const *Model::Get__End_Layer__Active(void) const {
      if (MODEL::AUTOENCODER == this->type)
        return this->ptr_last_layer -
    ...
    /* Strategy comparison index:
       ...
       || tr_validating <= td_validating && tr_testing < td_testing. [1]:
       tr_testing <= global_testing. [2]: tr_validating <= td_testing &&
       tr_testing < td_testing.
     */
    ...
    bool Model::Compare(
      ...
      switch (type_holdout_dataset_received) {
        ...
        case ENV::TESTG:
          ...
          break;
        default:
          ERR(L"Evaluation type (%d | %ls) is not managed in the switch.",
    ...
    void Model::layer_initialize_const_bias(real const bias,
      ...
      var const *const tmp_ptr_parameter_end(
          this->ptr_array_parameters + layer_it->last_bias_connection_index);
    ...
    void Model::lstm_initialize_const_bias(real const bias,
                                           Layer const *const layer_it) {
      size_t const tmp_number_cell_units(static_cast<size_t>(
          layer_it->ptr_last_cell_unit - layer_it->ptr_array_cell_units)),
          tmp_number_block_units(static_cast<size_t>(
      ...
      if (tmp_number_cell_units * tmp_number_block_units != 0_UZ) {
        ...
        for (tmp_ptr_parameter_end = tmp_ptr_parameter_it +
        ...
        // Forget gate.
        for (tmp_ptr_parameter_end =
                 tmp_ptr_parameter_it + tmp_number_block_units;
             tmp_ptr_parameter_it != tmp_ptr_parameter_end;
             ++tmp_ptr_parameter_it) {
    ...
    void Model::weights_initialize_uniform(
        var * ptr_array_weights_received,
        var const *const ptr_last_weight_received, real const lower_bound,
    ...
    void Model::lstm_initialize_uniform(real const lower_bound[5],
                                        ...
                                        Layer const *const layer_it) {
      BlockUnit const *tmp_ptr_last_block_unit(layer_it->ptr_last_block_unit);
      ...
      for (; tmp_ptr_block_unit_it != tmp_ptr_last_block_unit;
        ...
        for (tmp_ptr_last_cell_unit = tmp_ptr_block_unit_it->ptr_last_cell_unit,
            tmp_ptr_cell_unit_it = tmp_ptr_block_unit_it->ptr_array_cell_units;
             ...
             ++tmp_ptr_cell_unit_it) {
          // Input, cell.
          this->real_gen.range(lower_bound[0], upper_bound[0]);
          ...
          // Recurrent, cell.
          this->real_gen.range(lower_bound[2], upper_bound[2]);
        ...
        // Input, gates.
        this->real_gen.range(lower_bound[1], upper_bound[1]);
        ...
        // Recurrent, gates.
        this->real_gen.range(lower_bound[3], upper_bound[3]);
...
#ifndef NO_PEEPHOLE
        this->real_gen.range(lower_bound[4], upper_bound[4]);
    ...
    void Model::indrec_initialize_uniform(Layer const *const layer_it) {
      ...
      switch (layer_it->type_activation) {
        case LAYER_ACTIVATION::SYMMETRIC:
          tmp_lower_bound = -std::pow(
              tmp_MAG / std::pow(0.9_r, static_cast<real>(this->seq_w) / 10_r),
              1_r / static_cast<real>(this->seq_w));
          tmp_upper_bound = std::pow(
              tmp_MAG / std::pow(0.9_r, static_cast<real>(this->seq_w) / 10_r),
              1_r / static_cast<real>(this->seq_w));
          break;
        ...
        case LAYER_ACTIVATION::SELF_NORMALIZATION:
          tmp_lower_bound = 0_r;
          tmp_upper_bound =
              std::pow(tmp_MAG, 1_r / static_cast<real>(this->seq_w));
          break;
        default:
        ...
        if (layer_it->type_layer ==
          ...
          switch (layer_it->type_activation) {
            case LAYER_ACTIVATION::SYMMETRIC:
              tmp_lower_bound = std::pow(
                  1_r / tmp_MAG /
                      std::pow(0.9_r, static_cast<real>(this->seq_w) / 10_r),
                      1_r / static_cast<real>(this->seq_w));
              tmp_upper_bound = std::pow(
                  tmp_MAG /
                      std::pow(0.9_r, static_cast<real>(this->seq_w) / 10_r),
            ...
            case LAYER_ACTIVATION::SELF_NORMALIZATION:
              tmp_lower_bound =
                  std::pow(1_r / tmp_MAG, 1_r / static_cast<real>(this->seq_w));
              tmp_upper_bound =
                  std::pow(tmp_MAG, 1_r / static_cast<real>(this->seq_w));
              break;
            default:
              ERR(L"Layer activation type (%d | %ls) is not managed in the "
                  L"switch.",
                  layer_it->type_activation,
                  LAYER_ACTIVATION_NAME[layer_it->type_activation].c_str());
              return;
    ...
    void Model::weights_initialize_gaussian(
        var * ptr_array_weights_received,
        var const *const ptr_last_weight_received, real const variance) {
    ...
    void Model::lstm_initialize_gaussian(
        real const fwp_cell_var, real const fwp_gate_var,
        real const rec_cell_var, real const rec_gate_var,
      ...
      for (; tmp_ptr_block_unit_it != tmp_ptr_last_block_unit;
        ...
        for (tmp_ptr_last_cell_unit = tmp_ptr_block_unit_it->ptr_last_cell_unit,
          ...
          this->gaussian.range(0_r, fwp_cell_var);

          for (tmp_connection_index = 0_UZ;
               tmp_connection_index != tmp_number_feedforward_connections;
               ++tmp_connection_index) {
            tmp_ptr_array_parameters[tmp_connection_index] = this->gaussian();
          }
          // |END| Input, cell. |END|
          ...
          this->gaussian.range(0_r, rec_cell_var);
        ...
        // Input, gates.
        this->gaussian.range(0_r, fwp_gate_var);
        ...
        // Recurrent, gates.
        this->gaussian.range(0_r, rec_gate_var);
