...
namespace DL::v1 {
bool Model::Set__Accurancy_Variance(real const accurancy_variance_received)
{
    if(this->acc_var == accurancy_variance_received) { return true; }
    else if(accurancy_variance_received < 0_r)
    {
        ERR(L"Accuracy variance (%f) less than zero.",
            accurancy_variance_received);
        return false;
    }
    else if(accurancy_variance_received > 1_r)
    {
