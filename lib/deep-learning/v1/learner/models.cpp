...
bool Models::initialize_dirs(std::wstring const &name) {
  ...
  if (path_exist(home) == false) {
    ERR(L"Directory `%ls` not found.", home.c_str());
    return false;
  }
  ...
  if (create_directories(this->_path_root) == false) {
    ERR(L"An error has been triggered from the "
        L"`create_directories(%ls)` function.",
  ...
  if (create_directory(this->_path_trainer_dir) == false) {
    ERR(L"An error has been triggered from the "
        L"`create_directory(%ls)` function.",
...
bool Models::initialize_datasets(DatasetsParams const *const config) {
  ...
  if (this->_ptr_Dataset_Manager->Set__Maximum_Data(
    ...
    ERR(L"An error has been triggered from the "
        L"`Set__Maximum_Data(%d)` function.",
    ...
    return false;
             ...
             false) {
    ERR(L"An error has been triggered from the "
        L"`Preparing_Dataset_Manager()` function.",
        this->_ptr_Dataset_Manager->get_n_data());
...
bool Models::create(
  ...
  if (accept(L"Do you want to use template?")) {
    ...
    if (tmp_Neural_Network_Initializer.Template_Initialize() == false) {
      ERR(L"An error has been triggered from the "
          L"`Template_Initialize()` function.");
      return false;
    }
    ...
    ACTIVATION::TYPE const tmp_type_activation_function_hidden(
        static_cast<ACTIVATION::TYPE>(parse_discrete(
            1, ACTIVATION::LENGTH - 1, L"Hidden layer, activation function: ")));
    ACTIVATION::TYPE const tmp_type_activation_function_output(
        static_cast<ACTIVATION::TYPE>(parse_discrete(
      ...
      switch (
        ...
        case LAYER::FULLY_CONNECTED_INDEPENDENTLY_RECURRENT:
          if (this->_ptr_trainer_Neural_Network->set_layer_activation_function(
              ...
              false) {
            ERR(L"An error has been triggered from the "
                L"`set_layer_activation_function(%zu, %d)` function.", tmp_layer_index,
            ...
            ERR(L"An error has been triggered from the "
                L"`Set__Layer_Normalization(%zu, %d)` function.", tmp_layer_index,
            ...
            return false;
                     ...
                     this->_ptr_trainer_Neural_Network->set_dropout(
                         ...
                         std::array<real, 1_UZ>{
                             ...
                             .data()) == false) {
            ERR(L"An error has been triggered from the "
                L"`set_dropout(%zu, %d, %f)` function.", tmp_layer_index,
  ...
  if (accept(L"Do you want to use CUDA?")) {
    ...
    if (this->_ptr_trainer_Neural_Network->set_cu(
            this->_use_cuda, allowable_devc_mem) == false) {
      ERR(L"An error has been triggered from the "
          L"`set_cu(%ls, %zu)` function.",
          to_wstring(this->_use_cuda).c_str(), allowable_devc_mem);
      return false;
    }
  }
#endif
  ...
  INFO(L"");
  INFO(L"copy dimension and hyper-parameters into a neural network called "
       L"competitor.");
  if (this->_ptr_competitor_Neural_Network->copy(
          *this->_ptr_trainer_Neural_Network, true) == false) {
    ERR(L"An error has been triggered from the `copy()` function.");
    return false;
  }
...
bool Models::set_while_cond(
  ...
  if (while_cond.type == WHILE_MODE::LENGTH) {
    ERR(L"Type `%ls` is not valid.", WHILE_MODE_NAME[while_cond.type].c_str());
    return false;
  }
...
Model * Models::get_model(HIERARCHY::TYPE const hierarchy) {
  Model *model;
...
bool Models::append_to_dataset(real const *const ptr_array_inputs_received,
  ...
  if (this->_ptr_Dataset_Manager->push_back(
    ...
    ERR(L"An error has been triggered from the `push_back()` function.");
    return false;
  }

  if (this->_auto_save_dataset && this->_ptr_Dataset_Manager->save(
                                      this->_path_dataset_dir, true) == false) {
    ERR(L"An error has been triggered from the `save()` function.");
    return false;
  }
...
bool Models::evaluate_envs_pre_train(
  ...
  if (ptr_neural_network_received->Set__Pre_Training_Level(1_UZ) == false) {
    ERR(L"An error has been triggered from the "
        L"`Set__Pre_Training_Level(1)` function.");
    return false;
  }
  ...
  if (ptr_neural_network_received->Set__Pre_Training_Level(0_UZ) == false) {
    ERR(L"An error has been triggered from the "
        L"`Set__Pre_Training_Level(0)` function.");
    return false;
  }
...
bool Models::if_require_evaluate_envs(void) {
  if (this->require_evaluate_envs() && this->evaluate_envs() == false) {
    ERR(L"An error has been triggered from the `evaluate_envs()` function.");
    return false;
  }
...
bool Models::compare_trained(void) {
  ...
  if (tmp_updated) {
    ...
    if (this->_optimization_auto_save_trained &&
      ...
      ERR(L"An error has been triggered from the `save_model()` function.");
      return false;
    }
...
bool Models::compare_trained_pre_train(void) {
  ...
  if (this->_ptr_competitor_Neural_Network->Set__Pre_Training_Level(
          ...
          1_UZ) == false) {
    ERR(L"An error has been triggered from the "
        L"`Set__Pre_Training_Level()` function.");
    return false;
    ...
    bool const copy_to_competitor_received)
{
  ...
  if (path_exist(tmp_path_net) == false) {
    ERR(L"No such directory/file: `%ls`.", tmp_path_net.c_str());
    return false;
  }
  ...
  if (path_exist(tmp_path_nn) == false) {
    ERR(L"No such directory/file: `%ls`.", tmp_path_nn.c_str());
    return false;
  }
  ...
  if (model->load(
    ...
    ERR(L"An error has been triggered from the `load()` function.");
    SAFE_DELETE(model);
    return false;
  }
  ...
  if (copy_to_competitor_received) {
    ...
    if (this->_ptr_competitor_Neural_Network->copy(*model, true) == false) {
      ERR(L"An error has been triggered from the `copy()` function.");
      return false;
    }
  }

  if (model->set_mp(model->use_mp) == false) {
    ERR(L"An error has been triggered from the `set_mp(%ls)` function.",
        to_wstring(model->use_mp).c_str());
    return false;
  }

  if (model->set_cu(model->use_cu,
                    maximum_allowable_device_memory_bytes_received) == false) {
    ERR(L"An error has been triggered from the `set_cu(%ls, %zu)` function.",
        to_wstring(model->use_cu).c_str(),
...
bool Models::save_model(HIERARCHY::TYPE const hierarchy) {
  ...
  if (hierarchy == HIERARCHY::ALL || hierarchy == HIERARCHY::TRAINER) {
    ...
    if (this->_ptr_trainer_Neural_Network->save_params(tmp_path_net.c_str()) ==
        false) {
      ERR(L"An error has been triggered from the `save_params(%ls)` function.",
          tmp_path_net.c_str());
      return false;
    }
  ...
  if (hierarchy == HIERARCHY::ALL || hierarchy == HIERARCHY::TRAINED) {
    ...
    if (this->_ptr_trained_Neural_Network->save_params(tmp_path_net.c_str()) ==
        false) {
      ERR(L"An error has been triggered from the `save_params(%ls)` function.",
          tmp_path_net.c_str());
      return false;
