...
#pragma omp parallel private(i, batch_size)
  for (i = 0; i < n_batch; ++i) {
    batch_size = i + 1 != n_batch
                     ? batch_size_max
                     : n_data - static_cast<size_t>(i) * batch_size_max;
    ...
    this->model.forward_pass(
        batch_size, this->dataset.Xm + static_cast<size_t>(i) * batch_size_max);

    this->model.compute_loss(
        batch_size, this->dataset.Ym + static_cast<size_t>(i) * batch_size_max);
  }
